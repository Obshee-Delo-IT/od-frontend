...
| --- | --- | --- | --- |
| **od-dev** | `~/od-dev/public_html/` | `https://od-dev.tmweb.ru` (matches `redocly.yml`) | The instance this repo points at via `WP_BASE` in development. REST is on, content is migrated to Gutenberg, ACF is installed. ⚠ **Read it for approaches, never for content — established 2026-08-20.** How something is *wired* here is the reference: the six-widget footer, the `main-navigation` menu lookup, the ACF group's shape. What it *says* is not — its texts are months stale, and production is the article (od-stage is production's clone). So: copy the structure, take every word from prod. The footer proves the pattern — six block widgets is right, and lifting them wholesale still lost prod's «Учётный номер в реестре НКО», its «Электронная почта» wording and its relative privacy link. Any "what does WordPress contain?" question is answered against od-stage, and where a hand edit here has to exist on the real install it belongs in a script (`wp/scripts/od-wp.php`'s menu task is the pattern). |
| **od-stage** | `~/od-stage/public_html/` | `https://od.webtm.ru` (confirmed same install as this directory; the older `stage.od.webtm.ru` 301s to it, and only this name is covered by timeweb's `*.webtm.ru` certificate) | Staging, **writable since 2026-08-20** — wiped and re-synced from live prod that day, then taken **headless** the same day: WP **7.1**, `twentytwentyone`, **4 active plugins** (`contact-form-7` — since upgraded to 6.1.7 —, `cmsms-gutenberg-upgrade`, `wp-openapi` 1.0.29, `advanced-custom-fields` 6.8.8, with `leyka` 3.30.3 and `wp-downgrade` installed but **inactive**; re-read 2026-08-24) plus **four** mu-plugins (`od-profile`, `od-sidebars`, `od-film-meta`, `od-revalidate` — the last inert, no `OD_REVALIDATE_*` defines), **REST open** and `/wp-json-openapi` public, content migrated to core Gutenberg. Two repairs the theme deletion made necessary are in place: nav menu **39**'s slug is now `main-navigation` (the frontend's lookup key) and `sidebar_bottom` is re-registered, holding the six authored footer widgets. ACF's `group_film_meta` is imported and the film worksheet applied — 74 of the 85 catalogue films carry a `kinescope_id`. **Workstream D is applied** (`od-wp.php` then `od-pages.php`, 2026-08-21) and `[cmsms_*]` is gone from every published page; the dead-meta sweep of §4.4 step 8 has run — 215 469 rows, `cmsms_profile_subtitle`'s 193 deliberately kept. The orphan tables are still there on purpose: one of them is MailPoet's 10 013-address subscriber list ([runbook §0.6](./prod-migration-runbook.md) item 10). Plain `wp` works — none of §2's skip flags apply here. This is now the closest thing to the intended prod stack; the step log and what is still open are [`prod-migration-runbook.md` §0.6](./prod-migration-runbook.md). Per-step backups in `~/backups/`. |
...
**On a prepped install the flags are gone and two others take their place.** `contact-form-7` 5.4.2 and `leyka` 3.30.3 throw PHP 8.2 deprecations on every load, and they interleave with WP-CLI's own stdout — which makes `--format=csv` unparseable and scrolls a real error past unread. **Both are gone on od-stage as of 2026-08-24** — CF7 is 6.1.7 and leyka is inactive (and goes for good, [runbook §2.10](./prod-migration-runbook.md)), so plain `wp` there is clean — but keep the `-d` flags for any install still carrying either. The canonical invocation:
...
| `leyka_donation` | **leyka** | ✅ | | Donation records, 3070 of them. The plugin goes at cutover (runbook §2.10) and these rows stay as an archive — unregistered, so out of REST. |
...
- **Register it in an mu-plugin, not the theme's `functions.php`.** §4.5 offers either, and for the CPT the theme is the wrong home: a post type registered by a theme disappears the moment the theme changes, taking 205 rows out of the admin and out of REST with it. `wp/mu-plugins/` is already the convention in this repo, with the canonical copy under version control — mind its **PHP 7.4 floor**, since an mu-plugin loads on every site request and od-dev serves the site on 7.4.33 (measured 2026-08-20; prod's mu-plugin runtime is 8.2, so the lower one binds).
...
| **wp-openapi** | 1.0.21 (od-dev) / **1.0.29** (od-stage) | Generates the OpenAPI schema feeding `pnpm generate:types`. ⚠ **The “if it breaks under a WP update” case has arrived**, on WP 7.1: core's new `view-config` schema declares one property as `[]`, and openapi-typescript refuses the document — `#/components/schemas/view-config/default_view/filters: invalid property value. Expected Schema Object or boolean, got Array`. Rewriting that empty array to `{}` before generating is enough, and with that the 6.8.8 → 7.1 types still `type-check` clean, so no fallback to wp-graphql is needed — only a pre-pass. Measured 2026-08-20, [runbook §0.6](./prod-migration-runbook.md) item 3. |
...
| ~~**leyka**~~ | 3.30.3 | Donations. **Goes — decided 2026-08-24**: no `funded` donation since 2022-01-05 while 2603 `submitted` attempts kept arriving, and both «Оказать помощь» CTAs point at the external donation host. Deactivate and delete; the `leyka_donation` records stay. [Runbook §2.10](./prod-migration-runbook.md). |
...
### 4.4a What actually happened on od-stage (2026-08-20)

§4.4's ordering was written against od-dev, where the content had **already** been migrated. On a real prod clone it is wrong in one decisive way and right in the rest, so the executed order — with measurements, and the reason each step was forced — is [`prod-migration-runbook.md` §0.6](./prod-migration-runbook.md). Three corrections worth carrying back here:

- **Step 1 is obsolete.** `profile` + `pl-categs` + `cmsms_profile_subtitle` live in [`wp/mu-plugins/od-profile.php`](../wp/mu-plugins/od-profile.php), not in a theme's `functions.php` — deliberately, since a theme-registered post type vanishes with the theme.
- **Step 7 has a hard prerequisite step 7 doesn't state:** the migration must run *after* the core update, because the blocks it emits need 5.9+ (B10), and *before* cmsms is deleted only in the sense that the shortcode text must still be there — the plugin itself can be **deactivated** first and must be, since with `welfare` gone it fatals on every shortcode render (`cmsms_divpdel()` is a theme function).
- **Step 9 is moot on prod.** `carousel-block` is an od-dev plugin; prod has zero `wp:cb/carousel-v2` markup, so there is nothing to convert.
...
**27 active now** (26 at the 2026-05-29 audit, + ACF for B-VIDEO) → target **~6 after cleanup**: **ACF** (film fields — required, see §3.4), **CF7** (forms), **wp-openapi** (schema), **wp-optimize** (backend hygiene), **query-monitor** (dev-only), plus **wp-graphql** if it stays — **leyka is out** (2026-08-24, runbook §2.10). The `profile` CPT registration goes into the custom theme's `functions.php`, not into ACF.
...
| `GET /wp/v2/menus?slug=main-navigation` | `fetchMenus` | Main navigation (core `show_in_rest` since 5.9; needs `edit_theme_options`, so the application password is not optional here even though the nav is public). ⚠ **The slug is od-dev-only history.** On prod and on a fresh clone the same menu is term **39** with the slug `footer-navigation` — despite the name it is the *main* nav (`theme_mods_welfare.nav_menu_locations.primary = 39`), and `home` (40) is a leftover CMSMasters demo. Renaming the term is a required clone step ([runbook §0.6](./prod-migration-runbook.md) item 6). |
| `GET /wp/v2/menu-items?menus={id}` | `fetchMenuItems` | Menu nodes; `parent === 0` is root, `menu_order` is a depth-first walk (§6.3). |
| `GET /wp/v2/widgets?sidebar=sidebar_bottom` | `fetchFooter` | The footer, per the §6.3 widget pattern — this one is **built**, not just planned. `edit_theme_options` again. ⚠ `sidebar_bottom` was a `welfare` widget area, so deleting the theme empties it; [`od-sidebars.php`](../wp/mu-plugins/od-sidebars.php) re-registers the id and the four widgets are moved back into it (runbook §0.6 item 6). |
...
| `shared/lib/wpContent/resolveContentAssets.ts` | runs every body `<img>`, `<audio>` and media `<a href>` through `resolveMediaUrl`, strips `srcset` / `sizes`, and **sets `loading` itself** — eager on a main body's first image, lazy on every other one (see below) |
...
**`resolveContentAssets` does not trust WordPress's lazy-loading, and the reason is not image weight.** `wp_filter_content_tags` usually marks a body's images `loading="lazy"`, but not always — `/about/` comes back with six `<img>` carrying no `loading` attribute at all, which means eager. Next ships preload hints for non-lazy images inside a route's flight payload, so the App Router's **prefetch of a nav link executes them**: measured 2026-08-21 against a production build, every page on the site fetched `/about/`'s seven partner logos and certificates straight from the bucket — 7 cross-origin requests, the slowest 1.6 s, for images the visitor never sees. Aborting `/about/?_rsc=` alone took it to zero, which is what identified the cause. The helper now writes the attribute itself rather than inheriting whatever WordPress decided.
...
**Wired end to end on the stage tier — 2026-08-23.** The one hop nobody had exercised (WP → a *deployed* frontend over the network) is now proven: `config.php` sits on **od-stage** pointing at `https://new.obshee-delo.ru/api/revalidate/`, the Coolify application `od-frontend-stage` carries `REVALIDATE_SECRET` as a runtime variable, and the digests match on both sides. Measured in that order: the route answered **503** with no variable set, **401** with a wrong secret, **200** with the real one — `{"revalidated":{"tags":["wp:films","wp:post:74794",…,"wp:posts"]}}` — and the nine film pages served their new плакат on the next request instead of waiting out the hour. From WordPress itself, `wp eval 'OD_Revalidate::send( array( "tags" => array( "wp:films" ) ) )'` returns `true` with `configured()` also `true`.

Two things that made this necessary rather than optional on this tier: the stage application mounts a **persistent volume at `/app/.next/cache`**, so a redeploy does *not* drop the ISR cache — the only ways to refresh are the hourly window, a purge, or emptying that volume. And **Coolify's env is the frontend half**: without the variable the route is inert by design, which is what 503 means.

How the install was originally tested, before any frontend was deployed and on a shared host with no Node to run one: a listener on od-dev's loopback recorded exactly what the plugin sends for every content transition, and those bodies were then replayed against the real route on a production build. Both halves are proven; the one hop nobody has exercised is WP → a *deployed* frontend over the network. Record, with the captures: [`servers-agent/tasks/2026-08-13-od-revalidate-mu-plugin/`](../../servers-agent/tasks/2026-08-13-od-revalidate-mu-plugin/README.md).
