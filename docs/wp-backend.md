...
| --- | --- | --- | --- |
| **od-dev** | `~/od-dev/public_html/` | `https://od-dev.tmweb.ru` (matches `redocly.yml`) | The instance this repo points at via `WP_BASE` in development. REST is on, content is migrated to Gutenberg, ACF is installed. ⚠ **Not a source of truth for anything — established 2026-08-20.** Its content, its menus, its widgets and its field groups have been hand-edited here over months and none of it travels; production is the article, and od-stage is production's clone. Read it to develop against, never to decide what production contains, and never copy content or configuration *out* of it into a clone (a footer widget set was lifted that way and had to be reverted). Where an od-dev hand edit has to exist on the real install, it belongs in a script — `wp/scripts/od-wp.php`'s menu task is the pattern. |
