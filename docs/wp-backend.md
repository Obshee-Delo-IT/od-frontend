...
| **od-stage** | `~/od-stage/public_html/` | `https://od.webtm.ru` (confirmed same install as this directory; the older `stage.od.webtm.ru` 301s to it, and only this name is covered by timeweb's `*.webtm.ru` certificate) | Staging, **writable since 2026-08-20** — wiped and re-synced from live prod that day, then taken **headless** the same day: WP **7.1**, `twentytwentyone`, **4 plugins** (`contact-form-7`, `leyka`, `cmsms-gutenberg-upgrade`, `wp-openapi` 1.0.29) plus **three** mu-plugins (`od-profile`, `od-sidebars`, `od-revalidate` — the last inert, no `OD_REVALIDATE_*` defines), **REST open** and `/wp-json-openapi` public, content migrated to core Gutenberg. Two repairs the theme deletion made necessary are in place: nav menu **39**'s slug is now `main-navigation` (the frontend's lookup key) and `sidebar_bottom` is re-registered with its four footer widgets. Plain `wp` works — none of §2's skip flags apply here. This is now the closest thing to the intended prod stack; the step log and what is still open are [`prod-migration-runbook.md` §0.6](./prod-migration-runbook.md). Per-step backups in `~/backups/`. |
