...
| **od-stage** | `~/od-stage/public_html/` | `https://od.webtm.ru` (confirmed same install as this directory; the older `stage.od.webtm.ru` 301s to it, and only this name is covered by timeweb's `*.webtm.ru` certificate) | Staging, **writable since 2026-08-20** — wiped and re-synced from live prod that day, then taken **headless** the same day: WP **7.1**, `twentytwentyone`, **3 plugins** (`contact-form-7`, `leyka`, `cmsms-gutenberg-upgrade`) plus the `od-profile` mu-plugin, **REST open**, content migrated to core Gutenberg. Plain `wp` works — none of §2's skip flags apply here. This is now the closest thing to the intended prod stack; the step log and what is still open are [`prod-migration-runbook.md` §0.6](./prod-migration-runbook.md). Per-step backups in `~/backups/`. |
