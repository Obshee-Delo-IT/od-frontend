...
- [ ] **A3. CI pipeline.** Today there's Husky + lint-staged locally and Dependabot for deps. No CI runs lint / type-check / build on PRs. Add a GitHub Actions workflow that runs `pnpm lint`, `pnpm type-check`, `pnpm build` on PR.
...
- [~] **F1. Testing.** Rationale: AI-first development thrives on fast feedback — cheap unit/component tests catch regressions when an agent edits many files in one pass, far better than relying on manual review of each diff. Concrete stack: **Vitest** (unit + component, fast watch mode), **React Testing Library + jsdom** (behaviour-level component tests), **Playwright** (E2E for golden-path flows, leveraging the existing `playwright-extension` MCP). Initial setup lands Vitest + RTL with a couple of seed tests; Playwright added in a separate pass once a route or two are built. CI gate added in A3.
