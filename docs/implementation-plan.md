...
- [x] **A3. CI pipeline.** Deploy is wired via the Coolify GitHub app (triggers on push), so CI/CD's deploy half is already solved. `.github/workflows/ci.yml` runs `pnpm lint`, `pnpm type-check`, `pnpm test`, `pnpm build` on `pull_request` (any base) **and** on `push` to `main`. Concurrency cancels in-flight PR runs (latest commit wins) but lets every main-branch commit complete (so each deploy has its own pass/fail). pnpm pinned to 11.3.0 matching the Dockerfile; Node read from `.nvmrc`. The build runs **without** WP secrets in CI — `httpClient` and `next.config.ts` detect missing/empty `WP_BASE`/`WP_USER`/`WP_PASSWORD` and substitute a stub fetch that returns `[]`, so compilation + RSC boundaries are still validated end-to-end. Real-data builds remain Coolify's job, where the secrets live. **One-time repo-settings step (not code):** mark the workflow as a required check on `main` under Branches → Branch protection so Coolify never auto-deploys a red commit.
