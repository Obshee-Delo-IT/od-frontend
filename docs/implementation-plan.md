...
- [~] **D1. Home (`app/page.tsx`)** — frame `1622:10641` (desktop), `1356:15986` (mobile). Highest priority since `/` currently has no handler. **Desktop done** (commit `6f83bbd`): 8 sections composed — HeroBanner, stats row, films carousel, PromoBanner, Направления, Программы, Наши дела, SubscribeForm. WP-backed: films + latest news. Static content (stats, directions, programs, banner copy) lives in `src/modules/Home/constants.ts`. New primitives shipped: `LinkCard`, `StatItem`, `Carousel` (Embla), `NewsCard`, `FeaturedNewsCard`, `HeroBanner`, `PromoBanner`. Open follow-ups: (1) confirm films category ID — `FILMS_CATEGORY_ID = 0` placeholder in `src/shared/api/fetchFilms.ts`; (2) hero/promo placeholder artwork → real assets; (3) film cards need cover images; (4) news cards need cover images (requires `?_embed` or a 2nd media fetch); (5) mobile pass against frame `1356:15986`.
