...
1. **`plakati` · `zakladki` · `metodichki`** (partial D8) — 62 % of the section's entries for ~30 % of the build. `articles` ✅ already done as an alias, and the **`/materials/` index** ✅ shipped 2026-08-15 — it had only 107 entries but 939 views (8.8×), and was built for the second reason; it moved into its own WP page on 2026-08-18 (D6h).
