...
- [~] **A3. CI pipeline.** Deploy is wired via the Coolify GitHub app (triggers on push), so CI/CD's deploy half is already solved. **Done:** `.github/workflows/ci.yml` runs `pnpm lint`, `pnpm type-check`, `pnpm test`, `pnpm build` on `pull_request` (any base) **and** on `push` to `main`. Concurrency cancels in-flight PR runs (latest commit wins) but lets every main-branch commit complete (so each deploy has its own pass/fail). pnpm pinned to 11.3.0 matching the Dockerfile; Node read from `.nvmrc`. **Pending (one-time repo setup):** add three GitHub Actions secrets — `WP_BASE`, `WP_USER`, `WP_PASSWORD` — so the build step can fetch `generateStaticParams` posts. Until they're set, the build step will fail and the rest still gates. Mark the workflow as required on `main` in branch-protection settings once secrets are in.
