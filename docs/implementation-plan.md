...
- [ ] **C6. `Dropdown` / `Select`.** Figma `Dropdown Menu` (`1324:4234`, 5 variants), `cornerRadius:6` (not pill — distinct from Input), gray-4 stroke, chevron trailing. Used by video-filter. **Not satisfied by the existing header `ButtonGroupSubMenu`** (`src/shared/ui/components/ButtonGroup/`), which is a Radix `NavigationMenuSub` *nav-link flyout* («О НАС ⌄» / «МАТЕРИАЛЫ ⌄») — a different component. C6 is a single-select form control (pick one option → change a query). The header submenu is a useful Radix reference but distinct in role/markup.
