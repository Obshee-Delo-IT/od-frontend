...
- [ ] **A4. Observability.** **Yandex Metrica only** (GA is excluded under current Russian regulations, and was removed from prod on 2026-08-07). **Counter id `34478865`** — prod's own, so the new site inherits the history rather than starting a fresh counter. Wire into `app/layout.tsx` **unconditionally, as prod's own tag is** — F6's `CookieNotice` shipped 2026-08-24 and states rather than gates, which is parity with production and with the published policy (§15.3 sends refusal to browser settings). Procedure, including cutting the tag out of the frozen copy so one counter keeps one source, is [runbook §4.9](./prod-migration-runbook.md). **Error tracking deferred** — rely on standalone-server stdout/stderr until traffic justifies a tool.
...
- [~] **B8. WP cleanup — kill the page-builder and unsupported UI plugins.** Headless means WP only serves data, so most of the active plugins are dead weight. Target list and ordering in [`wp-backend.md` §4](./wp-backend.md).
  ...
  **B8a — `cmsms-content-composer` is gone (od-dev, 2026-08-18).** ✅ Deactivated **and deleted**, the second half being the point: three of its PHP files bootstrap WordPress themselves and answer unauthenticated HTTP whether or not the plugin is active — and **they still do on prod** ([`next-steps.md`](./next-steps.md)). Its registration now lives in `wp/mu-plugins/od-profile.php` (`profile` + `pl-categs` + the `cmsms_profile_subtitle` meta, `init` priority 20, `show_in_rest` added to the taxonomy so `/wp/v2/pl-categs` answers). The four published posts that still held `[cmsms_*]` were converted first, through four new branches in `cmsms-gutenberg-upgrade`, so prod's cutover conversion inherits them. **Unblocks D3**, and `/wp/v2/pl-categs` gives it a region filter it did not have. Detail and the verification list in [`wp-backend.md` §3.1](./wp-backend.md). Left: **the same three steps on prod** — convert the content, install the mu-plugin, then deactivate **and delete** — in that order and no other, since installing the registration after the removal leaves 205 records with no post type. [Runbook §2.6](./prod-migration-runbook.md) carries the commands and the verification list.
  ...
  5. Replace wysija-newsletters if newsletters are in scope. 6. ~~Decide on leyka.~~ **Decided 2026-08-24 — it goes**, with its records kept ([runbook §2.10](./prod-migration-runbook.md)). 7. Decide on wp-graphql.
...
- [ ] **B-VIDEO2. Finish populating film link data — Tier 0 blocker, but editorial, not code.** The tooling is done and exhausted. **Root cause: not every film post has video links.** The automated match is deterministic only through the YouTube bridge (`share_youtube` → source video title → its Kinescope entry, since the library is a 1-to-1 YouTube import). All **27** films with a `share_youtube` are matched, and **0 of the 29 still missing a `kinescope_id` have one**. Measured on od-dev 2026-08-05, **re-verified unchanged 2026-08-13** — no editorial data has arrived since:
  ...
  **Nine плакаты arrived as Яндекс.Диск folders (2026-08-23) and are now in the script.** The editor sent twelve folder links; a folder holds one плакат in six renditions — А2, квадрат, ютюб, PSD, TIFF, RAR — so which file the card gets is a human call, and the choice lives in `FOLDER_POSTERS` in `scripts/import-film-posters.mjs`: film id → `[folder link, path inside it]`. Keyed by **prod's ids**, which od-stage shares as its clone, so the same run reproduces these posters on prod after the cutover; od-dev descends from the same site, so its older posts share three of the entries — the same плакат for the same film — while ids minted since the fork fall through to the body/cover. An entry also fills **«Скачать плакат»** when `poster_download_url` is empty, with a deep link at the resolved path (`…/d/<key>?path=<file>`), so those cards carry the same download button as the films whose link the editor filled by hand; all seven were verified to open on the file itself. Paths are matched **Unicode-normalised** — the folders were authored on macOS, so their names are NFD and a typed NFC copy does not resolve (this cost an afternoon's confusion: `path=/плакаты как найти призвание` 404s while the listing shows exactly that string). Of the twelve links, eight films already had a плакат; the seven that landed — 74794 Сахар, 73381 Бактерии, 73084 Алкоголь. Взгляд изнутри, 72705 Как найти призвание, 19869 Табак, 19871 Алкоголь (Секреты манипуляции), 19123 Тайна едкого дыма — are all vertical А2, 1414×2000 to 5846×8268. **Two candidates were rejected after upload:** the library's `история одного обмана.png` (1509×663) and `почему же они курят.png` (3298×1878) are landscape banners, not плакаты, so 19864 and 34169 keep the featured cover, whose `-WxH` at least gives the card its own shape. Coverage on od-stage: a poster on **45 of 83** film pages, **29** of them a real full-size плакат (22 before this batch); a re-run reports `0 would be set`.
...
5. ~~**Yandex Metrica account access** for **A4**~~ — **not an open question: access is in hand** (the «Страницы входа» export `pnpm url:check` runs on comes out of that account), and the counter is **34478865**. So A4 is a build task, and the procedure — where prod's tag lives, why it is unconditional, and why it has to be *removed* from the frozen copy rather than left running — is [runbook §4.9](./prod-migration-runbook.md). The banner copy needed no sign-off either: it is prod's own published wording, and it shipped 2026-08-24.
