...
- [ ] **D7. Video / Фильмы (`app/video/`) — PAUSED 2026-06-04: blocked on the film data model (see B-VIDEO below).** Index `video` (`706:3315`, 2940×1440) + `video-filter` (`1554:17574`, 2968×1440, adds top-of-page `Dropdown Menu`) + player `video-page` (`1566:10433`, 3072×1440, has 3-card "related" carousel + share block) + mobile `video-page-mob` (`1567:10735` and `1567:11844`) + "Скачать фильм" download (`1581:10334`). Each video card carries its own social-share row (vk / logo-youtube / rutube — `Frame 33967`). Depends on C4 (pagination ✅), C5 (tabs ✅ — mobile tabs `Frame 33812/33813`), C6 (dropdown ❌ — the **desktop** category selector is a `Dropdown Menu` «Подобрать фильм по теме», not a tabs strip; tabs are the mobile switcher), C7 (checkbox ✅), and the download-asset strategy (E3 ✅ — links live in content).
  - **Why paused (data-model gap, confirmed against live WP 2026-06-04).** Film posts are `format=video` (`x-wp-total` = 203). They have **no ACF and no structured meta** (`meta` = `footnotes` only). Everything the Figma card needs beyond the basics — the VK/YouTube/Rutube share links, the dual «Полн./Сокр. версия» download buttons + their durations, the trailer, «О фильме» — lives as **free-form Gutenberg HTML in the post body** and isn't even consistently present (a sampled post had only two `disk.yandex.ru` links + a thumbnail in its content). So a faithful card can't be populated from structured data without a content-parsing heuristic (fragile) or new ACF fields. The clean, reliable fields are: `title`, featured image (thumbnail), `date`, `excerpt`, `categories`, `link`.
...
- [ ] **B-VIDEO. Film data model (WP-backend, blocks D7).** Decide how the per-film structured data is stored so the D7 card can render faithfully: ACF (or equivalent) fields for `download_full`/`download_short` (+ size/duration), `share_vk`/`share_youtube`/`share_rutube`, `trailer_url`, `about_url`. Alternative: a documented Gutenberg block/shortcode convention the frontend can parse deterministically. Needs WP admin/WP-CLI access (currently unavailable — see "What I'd need access to"). Until resolved, D7 ships only the pragmatic listing above, if at all.
