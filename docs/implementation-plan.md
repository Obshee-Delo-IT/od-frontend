...
- [~] **D3. About + team + profiles (`app/about/`, `app/profile/`). `/team/` ✅ shipped 2026-08-18** — Figma `team-1` + `team-1-mob`, and it needed no route: the page was already rendering natively, so the work was its body (eleven `/profile/…` links in an `od-team` grid), the eleven records behind it, and a two-row `pageSections.ts` for the tab strip it shares with `/about/supervisory/`. See [`implementation-notes.md` §3](./implementation-notes.md). Figma: `about` (`706:70`), `about-learn-more` (`706:1257`), ~~`team-1` (`706:1584`)~~, `team-2` (`708:3736`), `documents` (`706:3499`), `story` (`706:3568`), `Letters-of-appreciation` (`706:3602`), `charter` (`706:3695`), `Certificate` (`760:1662`) — 9 desktop frames, most with mobile variants. The live site has **11** sub-pages and they aren't the same 9 — resolve scope with Design. **Split by traffic:** `/profile/[slug]` is **Tier 2** (566 entries; the CPT is already Gutenberg, so it's one template over existing data); `/about/*` is **Tier 3** and leads it, being the most-*viewed* page behind the fallback. `/team/` was Tier 3 too and came early because `team-1` is the mock `PersonCard` was built against.
