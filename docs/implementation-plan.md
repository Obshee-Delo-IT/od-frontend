...
- [~] **D4. Contacts (`app/contacts/`) — Tier 2.** `contact` (`754:587`, accordion of contacts) + `contact-page` (`754:675`, page-header banner + a "Наши социальные сети" 3-card grid reusing the News Card). **The 75 regional pages are done** — `contact-page`'s branch card, coordinator `PersonCard`s two per row and the `h2` card titles all ship through `od-pages.php` (see the notes, D4 2026-08-20); what is left is the `/contacts/` index itself, the last high-traffic page on the A6 iframe. The accordion uses the same `Frame 33976/77` expand/collapse pattern as FAQ. Directory only — the live page has no form, so B6 is not a dependency. The existing `Accordion` needs an `Add Circle` expand-icon variant to match.
