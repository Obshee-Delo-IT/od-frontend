...
- [ ] **A7. od-stage → prod migration — Tier 0 blocker.** Procedure in [`prod-migration-runbook.md`](./prod-migration-runbook.md). **Three of the hard blockers are now closed as a procedure, rehearsed end to end on the prod clone (runbook §0.6, 2026-08-20):** REST is open (it took a *theme* deletion, not only the clearfy toggle — `welfare/functions.php` disabled it in code), core is on **7.1** so the migrated blocks actually render, and **all** of prod's content is off CMSMasters — 5393 posts / 144 pages / 132 profiles converted, 10 stray shortcodes left on 10 pages. Prod itself is untouched. **Still open:** ACF (B3), the application password + a `generate:types` diff across the 6.8.8→7.1 gap, per-environment category ids (B5), the media origin (B6), and prod's own PHP version — 7.4 is WP 7.1's floor and prod is `mod_php7` of unread minor. Ordered list in runbook §0.6.
