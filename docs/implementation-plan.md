...
- [ ] **D6d. Root-relative `/wp-content/…` links in bodies 404 on this site.** The mirror image of D6c: where that one turned WordPress-origin *absolute* links into paths we serve, these are already paths — `href="/wp-content/uploads/2020/04/22-scaled.jpg"` — and the file they name exists only on the WordPress host, so on the frontend every one of them is a 404. Measured on od-dev 2026-08-17: **12 724 links across 5 052 published posts, plus 286 across 37 pages.** That volume is why it belongs in the pipeline and not in `od-pages.php` — `resolveContentLinks` already knows the `/wp-` trees (it deliberately leaves absolute ones alone), so the fix is the same predicate run the other way: give a root-relative `/wp-` path the WordPress origin back. Doing it in code also means production needs nothing beyond a deploy, and posts are covered as well as pages. Lands with D4.
