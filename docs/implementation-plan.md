...
1. **`plakati` · `zakladki` · `metodichki`** (partial D8) — 62 % of the section's entries for ~30 % of the build. `articles` ✅ already done as an alias, and the **`/materials/` index** ✅ shipped 2026-08-15 — it had only 107 entries but 939 views (8.8×), and was built for the second reason.
...
- [ ] **B8. WP cleanup — kill the page-builder and unsupported UI plugins.** Headless means WP only serves data, so most of the **27** active plugins are dead weight. Target list and ordering in [`wp-backend.md` §4](./wp-backend.md). Critical path:
  ...
  7. *(optional)* drop dead `cmsms_*` / `nvp_content_copy` postmeta.
  8. Replace wysija-newsletters if newsletters are in scope. 9. Decide on leyka. 10. Decide on wp-graphql.
...
- [ ] **D6. Projects / Программы — the Figma detail templates only. Index ✅ 2026-08-15, programme pages ✅ 2026-08-15** (`app/projects/page.tsx` and the native-WP-page mechanism, see the notes — **every** WP page now renders natively at its own URL instead of through the A6 iframe, the programme pages included; `shared/config/legacyEmbedPages.ts` lists the 21 exceptions still pinned to the iframe). Still open: `project-1/2/3` (`759:845`, `759:1379`, `759:1117`) + long-form `article` (`778:1766`) + `article-mob` (`1567:11148`). Detail pages use a tab strip (`Frame 33787`, ~572×198 — C5) + a 3-card "project items" grid (`Frame 251` etc. with `Frame 151` content). **Traffic says this is smaller than the mocks imply** — zero `/projects/<slug>/` URLs in 91 days; the live «программы» are three top-level pages. **Confirm with Design whether `project-1/2/3` correspond to anything real before building three detail templates.** ~~Also settle the label~~ — the mock answers it: H1 «ПРОГРАММЫ», second section «Проекты». Left open by the index: the **wide-card variant** (598×280, illustration right) that the mock uses for «Бизнес-клуб» and «ОД ИТ» — unbuilt because both cards are hidden.
- [ ] **D6c. Dynamic blocks emit absolute links to the WordPress origin.** A page's `wp:query` output arrives from REST with `href="https://od-dev.tmweb.ru/<id>/"` on every card and a WP-origin `srcset` on every thumbnail — measured on `/contacts/chukotskiy-ao/`, 2026-08-17. The images are already handled (`resolveContentImages` rewrites `<img>` and strips `srcset`), **the links are not**, so a visitor clicking a news card on a regional contacts page leaves the site for WordPress. **86 of 170 published pages carry a `wp:query`**, ~80 of them the regional `/contacts/*` template, so this lands with D4. Fix in the content pipeline next to `resolveContentImages` — rewrite anchors whose origin is `WP_BASE` to root-relative — not in the content, because the markup is generated by WordPress at render time and no editor can change it. Note the same rewrite already exists for the A6 iframe in `src/shared/legacy/`, against a different origin and a different HTML string.
- [~] **D8. Materials (`app/materials/`) — partly Tier 2.** The biggest section. **The `/materials/` index is ✅ shipped** (frame `ads` `778:2206` — four static group cards; see [`implementation-notes.md` §3](./implementation-notes.md)); everything below is the sub-pages. Figma templates: `article` (`1012:10934`), `article-content` (`966:8461`), `handbooks` (`779:4133`), `books` (`966:6650`), `disks` (`966:8062`), `printing` (`966:2949`), `flyers` (`966:7747`), `social-ads` (`966:8538`), `social-posters` (`998:9524`), `social-sticker` (`1013:11191`), `social-banners` (`1009:10590`), `social-video` (`1012:11084`), `social-audio` (`1009:10756`), `car sticker` (`966:8388`). ~14 sub-pages, up to 88 assets per page. Depends on C4 ✅, C5 ✅, and the asset-hosting story (E1). **Split, don't build whole:** `plakati` · `zakladki` · `metodichki` are Tier 2; the other ten are Tier 3. `articles` is ✅ done as an alias. Note the index needed **no** answer to the materials-CPT question (it is four hard-coded links) — the sub-pages are where B1's content shape actually bites.
