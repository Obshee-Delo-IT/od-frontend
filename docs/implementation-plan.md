...
1. **`plakati` · `zakladki` · ~~`metodichki`~~** (partial D8) — 62 % of the section's entries for ~30 % of the build. `articles` ✅ already done as an alias, the **`/materials/` index** ✅ shipped 2026-08-15 (only 107 entries but 939 views, 8.8× — built for the second reason), and **`metodichki` ✅ shipped 2026-08-17** — the first page through the [`wp-page-redesign.md`](./wp-page-redesign.md) flow, so `wp/scripts/od-pages.php` and `PersonCard` now exist for the next two.
...
- [~] **B8. WP cleanup — kill the page-builder and unsupported UI plugins.** Headless means WP only serves data, so most of the active plugins are dead weight. Target list and ordering in [`wp-backend.md` §4](./wp-backend.md).
  ...
  **B8a — `cmsms-content-composer` is off (od-dev, 2026-08-18).** ✅ Its registration now lives in `wp/mu-plugins/od-profile.php` (`profile` + `pl-categs` + the `cmsms_profile_subtitle` meta, `init` priority 20, `show_in_rest` added to the taxonomy so `/wp/v2/pl-categs` answers). The four published posts that still held `[cmsms_*]` were converted first, through four new branches in `cmsms-gutenberg-upgrade`, so prod's cutover conversion inherits them. **Unblocks D3**, and `/wp/v2/pl-categs` gives it a region filter it did not have. Detail and the verification list in [`wp-backend.md` §3.1](./wp-backend.md). Left: **the same two steps on prod at cutover** (mu-plugin first, then deactivate — never the other way round), and `wp plugin delete` once prod's content has been migrated, since that plugin is the only reference implementation of the shortcodes prod still holds.
