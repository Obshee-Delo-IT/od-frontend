...
**Repo:** ✅ index — `app/projects/page.tsx` (2026-08-15): H1 «ПРОГРАММЫ» + a programme grid with no heading of its own + a «Проекты» grid, over `shared/config/programSections`, no breadcrumbs. **The section's "detail pages" are the three programme pages**, which render their own WordPress content natively at their existing URLs (D6b). `project-1` is now built as the template for those: `/healthy-russia/` was rebuilt against it 2026-08-17 — content by `wp/scripts/od-pages.php`, design in `gutenberg.css`, both shared with the two pages still to do (D6e in the notes). `project-2/3` remain unbuilt; see plan §D6.
