...
**Repo:** ⚠️ index shipped (D7, 2026-06-04) — `app/video/page.tsx` + `src/modules/Video/` (`VideoCard`, `VideoFilter`) + `fetchVideoList`, reading the `group_film_meta` ACF fields. Desktop = C6 `Dropdown`, mobile = C5 `Tabs`. Cards render downloads / share / watch only when the field is set (graceful degrade). **Still open:** the player page `/video/[id]` — E4 decided (embed **Kinescope**, НКО tier approved 2026-06-08); needs the `kinescope_id` ACF field + populated video IDs. See [`implementation-plan.md` §D7/§E4](./implementation-plan.md).
