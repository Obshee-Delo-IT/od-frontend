...
**Repo:** ✅ index — a WordPress page since 2026-08-18 (D6g), rendered by the catch-all: two rows of cards written into #59466 by `od_pages_projects()` and styled by `.od-tiles`/`.od-tile` in `gutenberg.css`. It shipped 2026-08-15 as `app/projects/page.tsx` over `shared/config/programSections` and that route is deleted; the H1 is the WP page title («Программы и проекты», where the mock says «ПРОГРАММЫ») and `WpPage` draws a breadcrumbs row the mock omits. **The section's "detail pages" are the three programme pages**, which render their own WordPress content natively at their existing URLs (D6b). All three are built: `/healthy-russia/` against `project-1` 2026-08-17 (D6e), `/healthy-youth/` against `project-2` and `/healthy-kids/` against `project-3` 2026-08-18 (D6f) — content by `wp/scripts/od-pages.php`, design in `gutenberg.css`, shared across the three. **`project-2/3` are not extra templates**: each mock's breadcrumb names its page («Здоровая Молодежь», «Здоровые Дети»), and each carries that page's own goal sentence. They differ from `project-1` only in numbering the task cards, sizing the row to the number of them, and setting the approval note as a standalone paragraph.
