...
**Repo:** ✅ index — `app/projects/page.tsx` (2026-08-15): H1 «ПРОГРАММЫ» + a programme grid with no heading of its own + a «Проекты» grid, over `shared/config/programSections`, no breadcrumbs. **The section's "detail pages" are the three programme pages**, which render their own WordPress content natively at their existing URLs (D6b). All three are built: `/healthy-russia/` against `project-1` 2026-08-17 (D6e), `/healthy-youth/` against `project-2` and `/healthy-kids/` against `project-3` 2026-08-18 (D6f) — content by `wp/scripts/od-pages.php`, design in `gutenberg.css`, shared across the three. **`project-2/3` are not extra templates**: each mock's breadcrumb names its page («Здоровая Молодежь», «Здоровые Дети»), and each carries that page's own goal sentence. They differ from `project-1` only in numbering the task cards, sizing the row to the number of them, and setting the approval note as a standalone paragraph.
