...
**Repo:** ⚠️ index shipped (D7, 2026-06-04) — `app/video/page.tsx` + `src/modules/Video/` (`VideoCard`, `VideoFilter`) + `fetchVideoList`, reading the `group_film_meta` ACF fields. Desktop = C6 `Dropdown`, mobile = C5 `Tabs`. Cards render downloads / share / watch only when the field is set (graceful degrade). **Still open:** the player page `/video/[id]` (needs the E4 player decision). See [`implementation-plan.md` §D7](./implementation-plan.md).
