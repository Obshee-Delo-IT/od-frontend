...
**«ОБЩЕЕДЕЛО-ПРО» is one nav entry with three addresses, and that is what `navOverrides` is shaped around.** The property's real home is `pro.obshee-delo.ru` («В начало | ОбщееДелоПРО»); od-dev's menu still points at `общеедело-про.рф`, whose DNS no longer resolves; and **prod's menu points at `od-pro.ru`** — live, but serving «Всероссийский конкурс социальных проектов», a different site (all three probed 2026-08-13). Keying an override on one host was therefore a bug that could only surface at the repoint: matching od-dev's spelling alone, it would have stopped applying against prod and put the entry back in the header carrying the contest URL. So an override lists **every** host its destination is known by, and `SHOW_PRO_IN_NAV` is the single switch that unhides it. The mechanism **fails open** — an override that matches nothing is simply invisible — which puts `hosts` on the `WP_BASE`-repoint checklist alongside the film category ids.
