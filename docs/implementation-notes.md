...
- **The task cards are numbered, not titled.** `project-1` gives each card an `<h3>`; these give it «01», «02», … in the same 32px red above the same 24px of space. That ordinal is the card's position in the row and nothing else, so it is a **CSS counter** on `.od-cards--numbered` rather than text in `post_content` — an editor who adds, removes or reorders a slide never renumbers anything, and «04» appears by itself. The number reuses the `h3` rule outright, since the mock draws them identically.
...
Everything else is the `project-1` template unchanged: the logo card, the goal card with its CSS-background drawing, the 120px block rhythm, the 45px heading gap, and — on `/healthy-youth/` only — «Проекты программы» as a `core/query` over a tag, `programma-zdorovaya-molodezh`, added to `wp/scripts/od-terms.php` with the programme's six films. `/healthy-kids/` has no such row in the mock and no films tagged for one, so it emits no query block at all; its transform ignores the term id the registry hands every page.
...
**Content the mocks have nowhere to put, kept anyway.** Both pages ended in `<h3>`s that were links — «Методические рекомендации» and «Фильмы программы» on one, «Методические материалы» on the other — and `/healthy-youth/` also carried a booklet cover above a «Скачать методичку PDF» button. Neither mock has a slot for any of it. Dropping a live download because a mock omits it is a worse outcome than a small deviation, so the links survive as a `core/buttons` block (`.od-materials`) after everything the mock does draw; the two decorative images do not. On `/healthy-youth/` the trailing heading pointed at the same Yandex Disk file as the button, so one button covers both.
...
**Fixed in passing, in `od-terms.php`.** `od_terms_alt_from_title()` stored the post title raw, and several film titles hold their guillemets as `&#171;`/`&#187;` — an alt attribute is escaped on the way out, so the entity would have been escaped a second time and read aloud as its own text. It is decoded before it is stored.

**The projects row needs плакаты, more visibly than on `/healthy-russia/`.** Only **one** of `/healthy-youth/`'s six films has one (`43846`, «Четыре ключа»); the other five letterbox their 16∶9 still in the dark band. The page's own uploads are the artwork the mock draws, and four of them are portrait at 366×517 — `plakats_2office_man.jpg`, `dirty-words.jpg`, `how-to-love.jpg`, `plakats_2office_woman.jpg` — so filling `poster_image_url` on those four films from the media library is data entry, not work. `way-of-hero.jpg` is landscape and needs a real плакат like the rest.

**Deviations carried over from D6e**, unchanged and still worth a word with Design: the red `<h1>` the mocks replace with the logo card, the poster pill carrying the film's title rather than «Подробнее», and the row's date-descending order — `project-2` draws its six films in the page's hand-picked order, and a query cannot reproduce that without a `menu_order` on the films.

`php wp/tests/od-pages.test.php` covers both transforms against real captured `post_content`, including that `/healthy-kids/` emits no query block and that neither page half-converts unrecognised input. Applied to od-dev and measured out of the DOM at 1440 and 375 against the frames' own bounding boxes: goal card 1240×404 (mock 402.83), tasks 387×240 ×3 and 600×189 ×2 (mock 387×239, 600×189), gaps 120 / 44 / 65 / 119 / 45, posters 387×546 at 40 apart, and two Swipers on the page with two mounted — no double init.

### D6g. `общее-дело.рф` counted as one of our own origins — 2026-08-17

*(The commit is `fix(D6e)` — it was written on a branch that had not seen the programme pages below, which took D6e and D6f. Renumbered here, not in the log.)*
