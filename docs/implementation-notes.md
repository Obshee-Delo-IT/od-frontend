...
**Where «Проекты программы» comes from, and the tag that will replace it.** Today it is four `core/image` blocks in the page, each linking to its film — the migrator's own output, re-read and re-tagged by the transform. The set is meant to be the programme's nine lesson films, and **nothing in the inherited model said which nine**: the candidates all sit in category `movies` (581) and share nothing else, and the tag that reads like the programme's, **72** «Здоровая Россия - Общее дело», is on ten *news* posts from 2015–16 and on no film at all.

So the tag is ours now — **`programma-zdorovaya-rossiya`** «Программа «Здоровая Россия»», created by the new `wp/scripts/od-terms.php` and applied to the six lesson films that exist ([`wp-backend.md` §3.2](./wp-backend.md)). **Three of the nine have no post on the site**, under any title, in any video category: «Влияние алкоголя на репродуктивную систему человека», «Алкоголь. Взгляд изнутри», «Наркотики. Медицинские и социальные последствия». The registry's docblock names them, so adding a slug is the whole job once they are published. One thing for content to settle: the alcohol lesson exists twice — **63287** «Алкоголь. Секреты манипуляции» (2021, has downloads, tagged) and **19871** «Секреты манипуляции — Алкоголь» (2016, no downloads, what the page links) — with different Kinescope ids.

**The row is not driven by the tag yet, and one thing blocks it:** the films' featured images are 1280×720 stills and this card is 3∶4, so a `core/query` + `core/post-template` would letterbox six landscape stills into portrait cards. That needs a portrait cover per film — a media job, not a markup one.
...
**Two things the page still needs from WP.** The methodology cover is a **210×300 upload** blown up to 386 wide, so it is visibly soft. And «Наркотики» is a 480×270 still where the other three covers are 366×516, so that one card letterboxes — `object-fit: contain`, because cropping to fit cut the film's title off the artwork. A first pass borrowed the film's printable плакат automatically when the page's own image was landscape; it was fifty lines of WordPress lookup for one image and came back out. Both are a better upload, not code.
