...
Three things Figma could not answer. Its map is one *merged* vector 991×572; ours is 82 separate paths whose drawn extent is 979×477, so the width matches the mock and the card comes out shorter — what is kept is the mock's *proportion*, air around the map at 9 % of its height against 8 %. The `viewBox` is **cropped to that extent** (`4 51 983 481`, measured once with `getBBox()`) because the plugin's declared 990×593 box carries 116 units of empty space that the card's own padding then compounds. And separate paths leave a pale anti-aliasing hairline on every internal border, which turned the flat silhouette into a visible region grid — closed with a hairline `stroke` in the *fill* colour and `vector-effect: non-scaling-stroke`, since the seam is one device pixel wide at any size. Hover and focus swap fill *and* stroke rather than drawing a contrasting outline: SVG paints in document order and has no `z-index`, so a border on an early region is clipped by its later neighbours.
