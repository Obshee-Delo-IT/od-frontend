...
- **Persistent volume on `/app/.next/cache`** — ISR pages and optimised images aren't in the image, so without it every redeploy cold-starts into a burst of requests against the slow WP plus full image re-encoding. Cache stays per-replica. **The corollary bit on 2026-08-22: a redeploy does not flush it.** The volume outlives the container, so a page whose ISR entry is younger than its `revalidate` is served from the *previous* deployment's cache — new code, old data. Measured that day: a WordPress category fix landed, `main` was pushed, the container was rebuilt and replaced, and `/video/` still listed the post that had been taken out of the category, while `/video/famous-people/` — which nothing had requested since, so it had no entry — rendered correctly off the new build. There is no deploy-time purge and there should not be one (the volume exists precisely so a deploy is not a cold start); the answer to «I changed WordPress, redeploy it» is **`POST /api/revalidate/`**, which is what `REVALIDATE_SECRET` is for, and the fallback is the hour.
...
**Three build-args and two BuildKit secrets.** `vars.WP_BASE`, `vars.WP_MEDIA_CDN` and `vars.SITE_URL` are args — none is secret, and all three are baked into the bundle. `secrets.WP_USER` and `secrets.WP_PASSWORD` go in through `secrets:` → `RUN --mount=type=secret` instead, and the distinction is not stylistic. They *are* needed at build time — that is the runbook's §4.7 warning, without them the build succeeds and prerenders an empty header and footer into every static page — but a **build arg is published**: buildx records every arg in the solve request's `frontendAttrs`, `docker/build-push-action@v6` uploads that record as a `.dockerbuild` workflow artifact by default, and this repository is public. Between 2026-08-21 and 2026-08-23 that put the od-stage application password, in cleartext, in nine downloadable artifacts — one per push to `main` — readable by any GitHub account. `provenance: false` was already set and did not help: the value was also inside an SLSA predicate embedded in the same bundle, and provenance is a different channel from the record. A secret mount appears in none of them and never reaches a layer. The job additionally sets `DOCKER_BUILD_RECORD_UPLOAD: false` as defence in depth, so a future `build-args:` line cannot leak the same way. Found by a security review, 2026-08-22; the password was rotated.
...
**Repo configuration** (GitHub → Settings → Secrets and variables → Actions): variables `WP_BASE`, `WP_MEDIA_CDN`, `SITE_URL`, `COOLIFY_APP_UUID`; secrets `WP_USER`, `WP_PASSWORD`, `COOLIFY_TOKEN`. The Coolify token is a dedicated `gh-actions-deploy` token (not the root one), minted with `php artisan tinker` and kept on the server at `/root/.coolify-ci-token`. ⚠ Coolify's `PATCH …/envs/bulk` still creates every key **twice** (the ops repo's known bug) — the duplicates were deleted by hand after the one bulk call.

**Three things blocked the first deploy, and none of them is in this repo.** All three were fixed on 2026-08-21; the ops-side detail is in [`servers-agent/tasks/2026-08-21-od-frontend-stage-deploy/`](../../servers-agent/tasks/2026-08-21-od-frontend-stage-deploy/README.md).

- **Coolify's API is IP-restricted out of the box** — `instance_settings.allowed_ips` ships as `10.0.0.0/8`, so every call from GitHub answered `403 {"message":"You are not allowed to access the API."}` while the same token worked over `localhost:8000`. Cleared to `NULL` (the API stays token-gated; without a token it is 401). GitHub's runner ranges are too many and too fluid to allowlist.
- **The token needs `read` as well.** `write` + `deploy` covers the PATCH and the deploy, but the completion poll is a `GET /deployments/{uuid}` and answered `403 {"message":"Missing required permissions: read"}`. Abilities are now `read,write,deploy`.
- **A public repo does not give you a public package.** The first push created `ghcr.io/obshee-delo-it/od-frontend` **private** (anonymous pull → 401) because the *organisation* forbade public packages — the package's own visibility control reads «Setting is disabled by organization administrators». Fixed at Organization → Settings → Packages, after which the VPS pulls with no credentials at all.

**Verified on the deployed tier, 2026-08-21:** container `healthy` (which is the `curl` change working — the probe is what turns a serving container green), running the **sha-tagged** image rather than `:stage`, 269 MiB of its 1 GiB, `.next/cache` owned by `nextjs` and already holding `fetch-cache` + `images`, and `pnpm url:check -- --base https://new.obshee-delo.ru` at **99.7 %** entry-traffic coverage — the same three known misses as the local container run (`/sms/`, which 404s on production too, and the two profiles that are `draft` there).
...
3. **The footer logo bypassed the image pipeline**, loading straight off the slow WordPress origin — the one rule CLAUDE.md states outright. It goes through `resolveContentAssets` now.
...
**Two config additions on the frontend.** `shared/config/pageSections.ts` is a table of pairs: it gives `/team/` and `/about/supervisory/` the tab strip the mock draws (they are unrelated pages in WordPress — different parents, no shared taxonomy) and the shorter H1 Figma uses, since the long WP title is what the tab itself says. *(A second pair joined it on 2026-08-20 — «Устав и документы»; see D6w.)* And `MAX_PROFILES_PER_PAGE` went from 8 to 16: the cap predated a page with a roster, and 11 links would have silently lost three.
...
- **`modules/WpPage`** — a news article without the news furniture: `resolveContentAssets` → `parsePost` → `GutenbergProvider`, a `PageHeader` on top (a WP page carries its title outside the body), no date and no similar-posts rail. That pipeline is shared, not copied — and the fourth consumer is what moved it out of `modules/News`, where three other modules were already reaching in: it now lives in **`shared/lib/wpContent/`** (`parsePost`, `resolveContentAssets`) and **`shared/ui/components/ImagePreview/`**. Names kept as they were; `parsePost` parses pages and films too, but renaming it is churn for no reader.
...
WordPress renders **absolute** hrefs. A page's `wp:query` loop arrives from REST with `href="https://od-dev.tmweb.ru/<id>/"` on every card (measured on `/contacts/chukotskiy-ao/`), and editors paste the same shape by hand. Images were already handled — `resolveContentAssets` rewrites `<img>` and strips `srcset` — **links were not**, so clicking a news card on a regional contacts page walked the visitor off the frontend and onto the CMS, which on production is the old site. **86 of 170 published pages carry a `wp:query`**, ~80 of them the regional `/contacts/*` template.
...
**The lightbox opened the thumbnail, not the poster.** On `/materials/plakati/` the `<img>` is a 270px-tall preview and the `<a>` around it points at the print-quality file — two different uploads, not two sizes of one — and the lightbox showed `img.currentSrc`, so clicking a poster got you the preview blown up. Two changes: the resolver now sends that href through `resolveMediaUrl` (it was a 404 before, stored root-relative against an origin that has no `wp-content`; nobody noticed because the lightbox swallowed the click), and `ImagePreviewClient` opens it. Its size cannot be measured — the file has not been fetched — so the ratio comes off the thumbnail and is scaled to a 1600px long side, otherwise `next/image` would serve a 187px-wide copy of the poster. Measured after: `Плакат-сколько-стоит-scaled.jpg` at 1200 natural in a 636×900 dialog, where it used to be 187×207.
...
### D6o. Two sizing fixes on the asset pages — 2026-08-18

**`/materials/sticker/`'s examples are four across, not two.** The mock draws them as two empty placeholders the size of a sticker card, and the four files behind them are 267 × 215 in the library — so the mock's row upscaled each photo to more than twice its width. Four across puts the box at roughly what the files are. The `od-figures` grid standing on its own is now `220px` tall with `object-fit: cover` and an 8px radius, because these are photographs that fill their box; inside a card it keeps the tall `420px` `contain` box, which is `/materials/zakladki/`'s 1:7 bookmark strips and the reason the fixed box exists at all.
...
**The open label is the stylesheet's, and that is the one thing a `<details>` can't do itself** (2026-08-20). A button still reading «Узнать больше» over an opened read looks like the click did nothing, and swapping the summary's text needs script this page hasn't got — so the stored text is the closed label and `.od-more[open] summary` folds it away with `font-size: 0` while `::after` draws «Свернуть». Two details that only show up in a browser: `line-height` has to be restated on the `::after`, because it inherits as a *number* and would otherwise multiply the 0; and the summary had to become a block-level `flex` box with `width: fit-content`, because as `inline-flex` it was baseline-aligned in a line box and the `font-size: 0` moved it 5px down on open. What a screen reader announces is the stored label in both states, which is the reason not to push the trick further — `<details>` announces expanded/collapsed on its own. Spacing came with it: 40px clear above and below in either state, which the lead gives up its own bottom margin for (`.od-lead p:last-child`) — before this the cards sat flush against the button with 3px between them.
...
**Eight cards, from the stored twelve tiles.** It shipped with eleven — every destination the tiles had — and **three came off on review the next day**, which is the call the frame can't make and the person who owns the section can: **Наблюдательный совет**, because `/team/` already carries the council, so the card was one destination twice (the team card says «Команда и наблюдательный совет» now); **Наша статистика**, because the site it points at is dated enough that linking it from the index undersells the organisation, and it comes back when that site does (`next-steps.md`); **«Оставь свой отзыв»**, which the footer links and which is still a Contact Form 7 page on the A6 iframe. Two of the twelve were never cards: «Видеопрезентация» is the page's own hero, «Нас поддерживают» the partner strip that replaces it. And the mock's own fourth card, «Отчеты», points at a page (`reports`, «Наши отчеты») that has been **private since 2017**, so its calendar drawing goes to «Документы» — the page that actually holds the reports — and nothing links to a page nobody can open. What's left is five wide cards and the mock's three portrait ones.
...
**One departure from the frame.** The mock draws no breadcrumbs and no H1 on this page; `WpPage` renders a `PageHeader` on every WordPress page and the page would otherwise have no heading at all, so it keeps one — in `next-steps.md`, along with the drawing «СМИ о нас» still borrows.

**Four more things the review changed, three of them site-wide** (2026-08-20):

- **The hero plays from Kinescope**, like every other clip this file writes: the stored `core/embed` was a YouTube frame, and the same video is in the Kinescope library under the identical title («Что такое ОБЩЕЕ ДЕЛО. Презентация организации»), so it is one row in `OD_KINESCOPE_EMBEDS`. `od_pages_kinescope()` needed one more url shape to read it — this page stores the `/embed/<id>` player url, where the twelve before it stored `watch?v=` or a `youtu.be/` short link.
- **«Подробнее» lost its underline everywhere**, and gained one on hover — which is what Figma draws and what nothing on the site was doing. The card version cost a *deletion*: the base `a` / `a:hover` pair at the top of `gutenberg.css` already says exactly that, so `.od-tiles .od-tile-link a { text-decoration: underline }` simply went. The hover is the **card's**, not the link's, for free — the stretched `::after` that makes the whole card clickable is part of the anchor's box, so `a:hover` matches wherever in the card the pointer is. `IllustratedCard` (the home page's carousel, the last React caller) went from `underline="always"` to `"hover"` for the same reason and by the same mechanism. Verified in a browser on `/about/`, `/materials/`, `/projects/` and `home`.
- **The card drawings are all one size now.** The four landscape `direction-*` borrowings went with the three dropped cards; «СМИ о нас», the one card the frame has no drawing for, borrows `/materials/`'s portrait «Статьи» file instead, so all five stand 200 tall in the card's 200 × 230 box.
- **The registration scans are centred.** Each is portrait in a 397-wide track, so `contain` at 400px tall leaves 80–105px of slack — and three documents of three different widths were lining up on their left edges, which reads as a mistake. `display: flex; justify-content: center` on the figure.

**«Устав» and «Документы» became one section, «Устав и документы»** (2026-08-20) — the same shape D3 built for «Команда» / «Наблюдательный совет», and the second pair in `pageSections.ts`. The two are one thing to a reader and were two cards, two menu items and two headings; now they are one card on `/about/` (into `/about/ustav/`), one WordPress menu item, and a tab strip that picks between them. The H1 and the last crumb follow the tab — «УСТАВ» on one, «ДОКУМЕНТЫ» on the other — exactly as the first pair's do, so the page says which half you are on and «Устав и документы» stays the name of the way in rather than a heading repeated twice. **Both URLs stay** — each has its own search traffic, and `<title>` and the canonical keep each page's own WP title; only the H1 and the trail belong to the section. Nothing about either body changed, so `documents` and `charter` are untouched. `.od-tile--about-docs::before` went with the card; the mock's calendar export stays in `public/figma/cards/about-docs.svg` in case «Отчеты» ever comes back.

**Three items came out of the WordPress nav menu** — «Написать отзыв», «Документы» (its half of the merge, with «Устав организации» retitled) and «Наша статистика», the card for which had come off `/about/` the same day. All of it is `od-wp.php`'s fourth task, `od_wp_edit_menu()`, not commands anybody has to remember: od-dev's database never travels to production, so a menu edited by hand in the admin is edited nowhere. Items are **found by the path they point at**, which is the one thing the two installs agree on — they disagree about the labels (prod says «Документы и отчёты») and about the origins (several items are still absolute against an old `.рф` domain, so the url can't be compared whole), and a nav menu item has no slug for the file's usual rule to use. `od_wp_menu_path()` is the pure part and is tested. **«Наша статистика» is the exception and is found by its label**, because its url is a bare domain — its path is `/`, which is «ГЛАВНАЯ»'s path, so a path match would have deleted the home link. Both installs carry that item under the same label and the same url. (`navOverrides` matched «ОБЩЕЕДЕЛО-ПРО» by label for the same reason, before that item was deleted outright — see §2 above.) Idempotent like everything else there: a deleted item reports «no item in main-navigation, skipped» on the next run, and the registry is checked for the two ways it can be wrong — a row with both matchers or neither, and a path of `/`.

The one thing that is still a runbook note is **when** to run it on prod: the merge leaves nothing linking `/about/docs/` for anything that does not draw the tab strip, and the old theme does not — so it belongs in the cutover window, which is where §2.8 already puts `od-wp.php`.
...
**Player 2026-07-02** — then at `app/video/[id]/page.tsx`; **A8 later moved it to `modules/Video/FilmPage` rendered at `/<id>`**, so read paths in this section as historical. SSG seed 20 + `dynamicParams`, `revalidate=3600`, `generateMetadata` incl. OG poster, `notFound()` for missing/non-`format=video` ids — all survived the move. New `fetchVideo`/`cachedFetchVideo`; a shared `mapVideoSummary` was extracted from `fetchVideoList`. New pieces: `FilmPlayer` (16:9 hero — Kinescope iframe when `kinescope_id` is set → poster linking out to `watch_url` → bare poster → nothing), `FilmActions` (download pills + trailer + share tiles; strip disappears when all empty), `RelatedFilms`, plus `absolutizeWpMedia` (film bodies use root-relative `/wp-content` srcs) and a shared `sharePlatforms.ts`. Body renders through `resolveContentAssets` + `parsePost`/`GutenbergProvider`. `next.config.ts` gained `staticGenerationRetryCount: 3` + `staticGenerationMaxConcurrency: 4` (od-dev 503s under the default parallel export).
...
The coordinator cards themselves are **two per row**, not the mock's single full-width one: `contact-page` draws a region with one coordinator, and `/contacts/samarskaya/` has eight. Inside the card, the person row keeps **30 above it** rather than the mock's 20, so a person and the two contact rows under them read as one lockup instead of four rows in a list.

Two content rules came out of the wider pages. A **label an editor typed in front of a link** goes when the glyph already says what the row is («ссылка Вк:», «тел.») and **stays** when it says something the glyph cannot («Отделение Вконтакте:» names *whose* page it is) — a rule loose enough to catch any short label took the meaning off the second kind, and read `https:` as a label besides. And a **contact stated twice is offered once**: several bodies name an address beside the coordinator and again on a line of its own, which came out as two identical rows under two identical glyphs.

**«События» is `/news/`'s card now** (`od_pages_branch_news`). The section was a four-column loop of cover, title, date and excerpt under core's unstyled «Предыдущая страница / 1 2 3 …» — the one place on the site where a post teaser was drawn by WordPress rather than by `NewsCard`, and it looked like it. It is now that card exactly: a 12px-radius cover at 280∶216, the date, the title, three across and surfaceless, with `.od-news-cards` in `gutenberg.css` restating `NewsCard.module.css`'s numbers for markup a CSS module cannot reach. The pagination is the site's own 36px chips, shared with the `.od-post-cards` pages through one `:is()` — a paginated `core/query` is paginated the same way wherever it is, and `/contacts/<region>/page/2/` already worked, being the same D3 mechanism `/about/smi/page/2/` uses. That it works *unambiguously* rests on a measurement: no region has more than twelve coordinators, so the coordinator loop never renders pagination of its own and `fetchWpPage`'s «first `query-<id>-page` in the body» is always the news loop's.

**Both card grids now share one builder** (`od_pages_card_query`), which is what keeps the two paginations from drifting: the white-card pages differ from the regional ones only in the blocks inside `core/post-template`.

### D4. The `/contacts/` index — a map that is links, and spoilers that are the region pages — 2026-08-20

The index was the last high-traffic page on the A6 iframe (597 views / 85 entries in 91 days), and it was there for two reasons at once.

**The map.** `/contacts/` drew a clickable map of Russia with **jqvmap**: jQuery 1.7.2 from a Google CDN, `../wp-includes/js/jquery.vmap.js` and `jquery.vmap.russia.js`, and an inline `vectorMap()` call whose only real content was `onRegionClick` — a `switch` over the region code doing `parent.location.assign("/contacts/<slug>/")`. Nothing about it survives natively: the `../wp-includes/…` paths resolve against *our* origin and 404, and `content.rendered` is parsed, not executed. What it was imitating is a link, so the rebuild is links — `src/modules/RussiaMap/`, an inline SVG of **82 `<path>`s, 70 of them wrapped in a real `<a href>`**, no client JavaScript at all, where the old page loaded ~255 KB of plugin to do the same job. Middle-click and «open in new tab» work for the first time.

The region table is generated (`pnpm map:generate`, `scripts/generate-russia-map.mjs`) from the plugin's own path data plus the published children of page 529, and the **order of its two inputs is the point**: the old `switch` first — 48 live cases and 15 commented-out ones, which are the editor's own code→URL decisions — then sorted-token-set matching of region names against page titles for the rest (62 via the switch, 7 by name, 1 by a written-down alias for Chechnya, which WordPress titles «Республика Чечня» and jqvmap «Чеченская Республика»). A wrong guess sends a visitor to a 404, so **an unmatched region is drawn unlinked rather than pointed at the nearest-looking page**, and the generator fails outright rather than emitting an href no published page answers for. Twelve regions are drawn and not clickable because no page exists (Дагестан, Липецкая, Калининградская, Марий Эл, Курганская, Ненецкий АО, Республика Алтай, Ингушетия and three of the North Caucasus republics); five pages have no region on a 2016 map and are reachable only from the accordion (Украина, ЛНР, ДНР, Беларусь and «Центральный Аппарат»). One old `switch` case went the other way: `case "sr"` pointed at `/contacts/saratovskaya/`, **a page that no longer exists** — a live 404 on the page being replaced.

Three things Figma could not answer. Its map is one *merged* vector 991×572; ours is 82 separate paths whose drawn extent is 979×477, so the width matches the mock and the card comes out shorter — what is kept is the mock's *proportion*, air around the map at 9 % of its height against 8 %. The `viewBox` is **cropped to that extent** (`4 51 983 481`, measured once with `getBBox()`) because the plugin's declared 990×593 box carries 116 units of empty space that the card's own padding then compounds. And the mock cannot answer the one thing a real map has to: **which regions you can click.** A merged vector has no regions, so the fills come from the mock and the structure from prod, which drew a 1px white `borderColor` between subjects, `colorRegion` for one it could route and a grey `selectRegion` for one it could not. So every region carries a white `stroke` — the internal edges are the subjects' own boundaries now, where before they were an anti-aliasing hairline to hide — and the twelve regions with no page are `--gray-4` instead of salmon, because a uniform silhouette offers a click that does not exist. `vector-effect: non-scaling-stroke`, since the border is one pixel at every size and in viewBox units it thinned to 0.16px at 390. Only the fill changes on hover: SVG paints in document order and has no `z-index`, so a contrasting *border* on an early region is clipped by its later neighbours.

**The spoilers.** Under the map sat **50 hand-written `wp:details` blocks**, each retyping a branch's legal name, its coordinator, a phone, an e-mail and a link — every one of which the region's own page already held, in the card [the branch-card pass](#d4-the-branch-card-on-75-regional-pages--2026-08-20) had just written there. Two copies of one fact drift, and this pair had: **25 of the 74 published regions had no spoiler at all**, three of the 49 that did linked to `общее-дело.рф` instead of this site, and nothing in the admin showed either. So the index now holds one shortcode. `[od_regions]` (`wp/mu-plugins/od-regions.php`) lists the published children of the page it sits on, renders **75** disclosures from their titles, permalinks and cards, and the page body went from **102 105 bytes to 889**. Adding a region to the index is now publishing its page.

**Why a shortcode and not a `core/query` loop**, which is how every other repeated thing on this site is drawn: the design is a *disclosure*, and core has no accordion block. `core/details` is the block that renders one and it cannot be driven from a loop — its `summary` is declared `{"type":"rich-text","source":"rich-text","selector":"summary"}` with no `"role":"content"`, so the Block Bindings API will not bind it, and core's only binding sources are `core/post-meta` and `core/pattern-overrides` anyway, neither of which can produce a post title or a permalink. Putting the contacts *inside* the disclosure has the same problem one level down: `core/post-content` would inline the whole regional page, «События» loop and all. There is no rung between «a class and some CSS» and a shortcode here.

Four details worth knowing about it:

- **The card is lifted, not rebuilt** — `parse_blocks()` + a depth-first `className` token match + `render_block()`, because the class is JSON in the block's attributes and the `.wp-block-group__inner-container` the D4 CSS keys on is added at *render* time. So `.od-region` in `gutenberg.css` styles only what is new — the pill, the ⊕/⊗ and the link — and everything inside the panel is the `.od-branch` / `.od-contact` rules that already existed.
- **`/contacts/sverdlovskaya/` had no card**, and that showed: it never had an «Об отделении» accordion for `od_pages_branch_card()` to convert, so its spoiler opened onto nothing but a link. The page writes its nine people straight into the body instead — a centred legal name, then two 50 % columns of «**role:** name / тел. / e-mail» — which is the same shape every accordion holds, minus the wrapper. So the transform gained a fallback (`od_branch_card_from_body`) that reads the body's own prefix, bounded by **the first `wp:query`**: everything before the group wrapping it is the branch's text, everything after is the coordinator loop and «События». It returns the page unchanged unless that prefix reads as contacts, so a page with no query block or a prose opening is not rewritten. Four role words came with it — «лектор», «соучредитель», «член правления», «ответственный» — which the earlier list did not have because no accordion used them; the page now draws 8 person lockups and 32 contact rows on the index and on itself. What stays prose there is what the rule says should: «Группа в ВК:» and «Почта отделения:» name *whose* they are, and a glyph cannot say that.
...
One more shape came out of «запятою убери»: `/contacts/moscow/` states two roles in bold with a comma between them («**Руководитель департамента профилактики**, **Руководитель Московского городского отделения** Моисеев Олег Олегович»), and reading only the first left the comma and the second title inside the man's name. The matcher now consumes bold runs until the line stops opening with one.
...
**19 cards hold only the legal name, and one page was drafted for it.** The pages behind those 19 say only that much — «Адрес офиса:», «тел.», «e-mail:» with nothing after them, confirmed against each page's pre-conversion revision — so the emptiness is the data. What stopped it from becoming 19 unpublished pages is that **an empty card is not an empty page**: `/contacts/arkhangelskaya/` lists 8 coordinators and 50 events under its empty card, and all but one of the 19 list events. `od_wp_draft_empty_branches()` therefore requires all three to be missing — no contact in the body, no coordinator in the `pl-categs` loop, no post in the `category` loop — and on od-dev exactly one page qualifies, `/contacts/evreiskaya-ao/`. Drafting is not deletion but it is not free either: the catch-all stops finding a published page for the URL and falls through to the A6 iframe, or to a 404 on a tier with no `WP_LEGACY_BASE`. That page drew 0 views in 91 days. The other 18 are in [`next-steps.md`](./next-steps.md) as a content gap.

⚠️ **And od-dev's page set is not production's**, which is where this had to be checked from. Production's `/contacts/` subtree: **58 published children and 2 drafts** against od-dev's 74; production publishes `rezan-oblast` and `smolenskaya-oblasti` (od-dev 404s both, and the census had written them off as slugs that never existed); od-dev carries ~16 regional pages production has never had; **5 cards are empty on production**, not 19, and od-dev's `khakasiya` has contacts production's does not. The launch runs against production's set, so every count in [`page-inventory.md`](./page-inventory.md) is the shape of the work rather than the shipping list — §8 there now says so.
...
1. **`/materials/autosticker/` was not in the registry at all.** Its record and `/team/`'s were written as *one* array literal, so PHP kept the later `label`/`path`/`fix` and the autosticker entry vanished — a registry one record shorter, nothing malformed, nothing warned, and the page counted as redesigned in every doc while never having been transformed. od-dev's copy turned out to be in shape (D6m had run before the literals were merged), so the fix is only the split and a test: **every transform the file defines is registered**, recognised by the `(string $content, int $x)` signature the runner calls. A count of the registry against itself proves nothing here.
2. **«An empty paragraph block» has three spellings**, and they are one page at three moments in its life: the editor writes the void `<!-- wp:paragraph /-->`, `wp_update_post()` rewrites it as the paired `<!-- wp:paragraph --><!-- /wp:paragraph -->` on save, and the CMSMasters migrator wrote `<p>&nbsp;</p>` between the tags. `od_drop_empty_layout_groups()` knew the third; the first conversion of `/contacts/` therefore left a trailing empty group standing, and the next run called the page already in shape. It now matches all three, and the transform's «already converted» guard runs the layout sweep instead of returning early, so a page converted by an older version can still be cleaned.
...
### F4b. The favicon and the social card, 2026-08-22

Two things a visitor sees before they see the site: the tab icon and the card a shared link unfurls into. Neither existed — `app/favicon.ico` was still **Next's own Vercel triangle**, and no route emitted an `og:image`, so a link pasted into Telegram or VK rendered as bare text.

- **The icons come from production's own logo**, `wp-content/uploads/2016/03/od-250x250.png` — the icon the live site has served for a decade, so nothing about the tab changes at cutover. Three files under `app/`, all file-convention: `favicon.ico` (a 48/32/16 multi-resolution `.ico`, since a browser picks the size it wants and a lone 250px source is resampled badly at 16), `icon.png` (192) and `apple-icon.png` (180, alpha removed — iOS composites a transparent icon onto black).
- **`public/og-default.png`** — 1200×630, the aspect every network crops to: «ОБЩЕЕ ДЕЛО» in PT Sans Narrow Bold on brand red (`#be1710`), over a letter-spaced «ОБЩЕРОССИЙСКАЯ ОБЩЕСТВЕННАЯ ОРГАНИЗАЦИЯ», the site's own H1 as the strapline, and the domain. **Typography and no photograph on purpose** — a feed renders the card at ~300 px, where a headline still reads and faces in a photo collage do not. 14 KB as a 64-colour PNG8. Chosen from three (hero-strip, red block, photo collage) built and compared as an artifact; the other two are in `/.scratch/og-variants/`. The type is the site's own PT Sans Narrow Bold / PT Sans Bold — the TTFs came from `google/fonts` for the one-off ImageMagick composition and are deliberately not in the repo, so **re-cutting the card means fetching them again**, not editing a source file.
- `OG_DEFAULT_IMAGE` in `shared/config/site.ts` holds the path, **relative**, so `metadataBase` absolutises it per tier rather than hard-coding prod.
- **Seven more cards name their section** under the wordmark — `og-news`, `og-articles`, `og-video`, `og-filmy`, `og-multy`, `og-roliki`, `og-famous-people`. An index has no photograph of its own and borrowing a post's would advertise that post, so the card says where the link goes instead; the five catalogue ones live in `CATALOGUE_COPY` beside the rest of its per-segment copy, which is also what stopped `/video/filmy/` and `/video/multy/` sharing one card.

**Two defects the first cut shipped, both found by reading what the cards actually pointed at (2026-08-23).**

- **`/contacts/` advertised a URL nothing answers.** All four images in its body are `<img src="image/png;base64,…">` — a data URI with the **scheme cut off**, so it looked relative and resolved against the WordPress origin into `https://od.webtm.ru/image/png;base64,…`. `extractFirstImage` now skips inline blobs (both the proper `data:` form and the truncated one) and returns the next real file, which fixes the card and the list thumbnails that share the helper. `/contacts/` has nothing else, so it lands on the branded card.
- **`/about/` advertised АСИ's logo.** Its body opens with a partner row — АСИ, ФСИН, МО, Татарстан — then scans of the registration certificates, so *no* image in it should represent the page and skipping to the next one only picks another partner. `LOGO_STRIP_PAGES` in `WpPage.tsx` names the two pages shaped like that. The heuristic itself stays, and the measurement is why: over 30 pages the body's first image is a sensible one on 22 of the 26 that have one (`/materials/plakati/` gets its poster, `/contacts/moscow/` its coordinator's photo). A page's own featured image would be the better source and does not exist — **1 of 100 pages has one, against 100 of 100 posts**.

- **So the news card takes `featured_media`, not the body** (2026-08-23), with the body's first image as the fallback for the post that has none: the featured image is the one someone chose for the post, where the body's first is whatever the layout opens with. It costs one extra request — `fetchFeaturedImage`, a raw `wpFetch` because `wp-openapi` omits `/wp/v2/media` from the schema entirely, `_fields=source_url`, wrapped in `cache()` and tagged `wp:post:<id>` so a post edit purges the lookup with the page. Switching `fetchNews` to `_embed` would have avoided the request and cost the whole post type its generated typing.
- **Every route that declares `openGraph` names an image**, and the ones with their own content image prefer it: a film keeps its poster, a news post takes its **featured image** and a WP page the body's first one, both through `resolveMediaUrl`, a profile takes its photo, and the indexes fall back to the logo.

⚠️ **Not an `app/opengraph-image.png`, on purpose.** The file convention looks like the obvious answer and is the wrong one twice over: Next merges metadata **shallowly**, so a segment's `openGraph` replaces the parent's object wholesale and inherits no image anyway, and **file-based metadata outranks config-based** — a root-level file would stamp the logo over every film poster and news photo, which are the only cards that matter. Hence the constant, repeated at each of the eight declaration sites. Verified against the rendered HTML, not the docs: `/71933/` unfurls its film cover, `/72897/` its lead photo, `/about/` its first body image, `/materials/` the logo.
...
**Re-measured 2026-08-20, and the table above is now historical.** D3 and D6 did not move traffic, they moved *pages*: ~150 of them came off the iframe onto native rendering at the same URLs. The same two exports, re-binned by `pnpm pages:inventory`, put natively-rendered URLs at **97.0 % of views and 98.9 % of entries**, and the A6 iframe at **1.6 % / 0.5 %** over its remaining six paths — the last figure after D4 took `/contacts/` off the list the same day. Keep the 2026-08-13 numbers for reading the launch tiering, which was priced off them; use [`page-inventory.md`](./page-inventory.md) for anything current.
