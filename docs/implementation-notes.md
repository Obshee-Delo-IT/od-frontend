...
**«Написать отзыв» came out of the WordPress nav menu**, and the merge took its two items with it — «Устав организации» retitled, «Документы» deleted. Both edits are `od-wp.php`'s fourth task, `od_wp_edit_menu()`, not commands anybody has to remember: od-dev's database never travels to production, so a menu edited by hand in the admin is edited nowhere. It is **keyed by the path each item points at**, which is the one thing the two installs agree on — they disagree about the labels (prod says «Документы и отчёты») and about the origins (several items are still absolute against an old `.рф` domain, so the url can't be compared whole), and a nav menu item has no slug for the file's usual rule to use. `od_wp_menu_path()` is the pure part and is tested. Idempotent like everything else there: a deleted item reports «no item in main-navigation, skipped» on the next run.
