...
**Section heading here is PT Sans Bold 32** (`text/8/bold`), not the 48 PT Sans Narrow the home sections render. Worth knowing that **home is the one off-mock**: `3614:91190` («Направления деятельности») is also 32 Bold, and `Directions.module.css` still says 48. Left alone — D1 is signed off and this is a visible change to it; it is a one-line fix when someone wants it.

**Cards are now paired to drawings by name, not position.** Figma gives each card a fixed illustration on both pages — «Общее дело ПРО» is the charts drawing in `projects` *and* in home's `Направления деятельности` (`3614:91189`) — but the code held a positional `ILLUSTRATIONS` array, so the home page drew the wrong picture for it, and `/projects/`'s two sections both restarted at `direction-1`. Each card in `programSections` now carries its own `Illustration`, and three drawings that had no asset (the programmes': `healthy-russia`, `healthy-kids`, `healthy-youth`) were exported from `706:1775` as SVG. `direction-1/3/4.svg` are now unreferenced — they belong to the three hidden cards.
...
**Not built: the wide-card variant.** The mock's middle row splits five project cards 2 + 3, the first two being 598×280 cards with the illustration on the right (`Frame 33845/33846`). Both of those cards — «Бизнес-клуб» and «ОД ИТ» — are hidden, so the variant would render for nobody. It comes back with them.
...
**Also settled:** outbound HTTP from WP works on Timeweb (200 in 0.26 s to an external host, from both CLI and web requests), which was an open question in `wp-backend.md` §8 and the last thing that could have made this impossible.
