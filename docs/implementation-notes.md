...
- **The film posters keep «Подробнее».** Four identically-named links would normally read badly, but the poster above each one is itself a link carrying the film's title as its alt text, so the card announces the film either way.
...
**Where «Проекты программы» comes from, and why it is not a tag query.** The four films are four `core/image` blocks in the page, each linking to its film — the migrator's own output, re-read and re-tagged by the transform. A `core/query` block filtered by a tag would be the better shape and is not available: **no tag joins these films.** All four are in category `movies` (581) and nothing else common; the tag that reads like the programme's, `72` «Здоровая Россия - Общее дело», is on ten *news* posts from 2015–16 and on none of the films. Their featured images are 1280×720 stills, so even with a tag a post-template would render the wrong shape into a 3∶4 card. Making it a query means, on the WordPress side first: a programme term applied to the films, and a portrait cover per film. Until then the carousel is the editable surface.

**The one landscape cover is fixed from the film, not from the page.** «Наркотики» is a 480×270 still where the other three are 366×516, and in a 3∶4 card it letterboxed. `od_pages_wp_portrait_cover()` — passed into the transform as a callback, so the transform itself stays a pure function of a string — keeps the page's own image when it is already portrait, and otherwise borrows the film's `poster_image_url`, the printable плакат, when *that* is portrait. For 22289 that is a 1810×2560 poster. Eleven films carry the field, so this fires rarely and only where it helps; `object-fit: contain` stays, because a cover that is landscape and has no better twin must still be shown whole. Finding it needs one workaround: attachments imported from the old punycode domain keep a `guid` that matches no URL this site serves, and WordPress attaches an oversized upload under its `-scaled` copy while the field still points at the original — so the lookup falls back to matching `_wp_attached_file` against both file names.
...
**One thing the page still needs from WP.** The methodology cover is a **210×300 upload** blown up to 386 wide, so it is visibly soft — it needs a bigger file, not a CSS change.
