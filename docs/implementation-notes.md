...
- **`/about/` advertised АСИ's logo.** Its body opens with a partner row — АСИ, ФСИН, МО, Татарстан — then scans of the registration certificates, so *no* image in it should represent the page and skipping to the next one only picks another partner. `LOGO_STRIP_PAGES` in `WpPage.tsx` names the two pages shaped like that. The heuristic itself stays, and the measurement is why: over 30 pages the body's first image is a sensible one on 22 of the 26 that have one (`/materials/plakati/` gets its poster, `/contacts/moscow/` its coordinator's photo). A page's own featured image would be the better source and does not exist — **1 of 100 pages has one, against 100 of 100 posts**, which is also the note for whoever revisits the news card.
- **Every route that declares `openGraph` names an image**, and the ones with their own content image prefer it: a film keeps its poster, a news post and a WP page take the body's first image (`extractFirstImage` → `toFullSizeImageUrl`, the same pair the cards use — the `-WxH` variants 500 on the media CDN), a profile takes its photo, and the indexes fall back to the logo.
