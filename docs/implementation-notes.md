...
**«Устав» and «Документы» became one section, «Устав и документы»** (2026-08-20) — the same shape D3 built for «Команда» / «Наблюдательный совет», and the second pair in `pageSections.ts`. The two are one thing to a reader and were two cards, two menu items and two headings; now they are one card on `/about/` (into `/about/ustav/`), one WordPress menu item, one H1 on both pages, and a tab strip that picks between them. **Both URLs stay** — each has its own search traffic, and `<title>` and the canonical keep each page's own WP title; only the H1 and the trail belong to the section. Nothing about either body changed, so `documents` and `charter` are untouched. `.od-tile--about-docs::before` went with the card; the mock's calendar export stays in `public/figma/cards/about-docs.svg` in case «Отчеты» ever comes back.

**«Написать отзыв» came out of the WordPress nav menu** (item 27991, od-dev, 2026-08-20) — the same call as the `/about/` card the day before, and for the same reason: the footer's «Отзывы» column links the page, so the nav entry was a third route to a Contact Form 7 form. **Not done on prod**, which is still the live site under the old theme: taking a working entry out of the live nav before cutover is a content decision, not a migration step, so it is in the runbook §2.8 as one. The same holds for merging prod's «Устав организации» and «Документы» items — prod draws no tab strip, so merging them there would leave its readers with no link to `/about/docs/` at all.
