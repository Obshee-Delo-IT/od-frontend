...
- `core/post-featured-image` writes **`object-fit` inline**, which no stylesheet can beat, and its default is `cover` — a 16∶9 still cropped to a 3∶4 card, title and all. The block's own `scale: contain` is the fix.
...
**What the row needs to look like the mock:** portrait covers. All six featured images are 1280×720 stills, so in a 3∶4 card each sits in a band with white above and below. `contain` rather than `cover` is deliberate — cropping cuts the film's title off the artwork — and the card keeps the mock's ratio so it comes right the moment a portrait cover is uploaded.
