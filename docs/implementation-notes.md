...
- **The film posters are a four-up grid, not a carousel.** The mock draws six items and arrows; the page has four, and a carousel for four cards is machinery with nothing to carry. The card itself is the mock's: the poster fills it at radius 12 with a white pill floated 12px off the bottom of the artwork. The pill says «Подробнее», as the mock does — four identically-named links would normally read badly, but the poster above each one is itself a link carrying the film's title as its alt text, so the card announces the film either way.

**Two things the page needs from WP, not from here.** The «Наркотики» cover is **landscape** where every other poster is portrait, so that one card letterboxes (`object-fit: contain` — cropping to fit cut the film's title off the artwork); swapping it for the film's own vertical poster is a media edit. And the methodology cover is a **210×300 upload** blown up to 386 wide, so it is visibly soft — it needs a bigger file, not a CSS change.
