...
`php wp/tests/od-pages.test.php` covers both transforms against real captured `post_content`, and `php wp/tests/od-films.test.php` checks the registry for the typo it can actually have — a film listed twice under one tag, or a плакат written as an absolute URL. Applied to od-dev and measured out of the DOM at 1440 and 375 against the frames' own bounding boxes: goal card 1240×404 (mock 402.83), tasks 387×240 ×3 and 600×189 ×2 (mock 387×239, 600×189), gaps 120 / 44 / 65 / 119 / 45, posters 387×546 at 40 apart, and two Swipers on the page with two mounted — no double init.
