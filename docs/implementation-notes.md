...
**Section heading here is PT Sans Bold 32** (`text/8/bold`), not the 48 PT Sans Narrow the home sections shipped with. **Home was the one off-mock**, and `Directions.module.css` was corrected to 32/24 on 2026-08-15 (`3614:91190`). `FilmsCarousel` and `NewsGrid` are still 48 PT Sans Narrow and Figma says 32 Bold for them too (`3614:91093`) — left for a decision, since it moves two more signed-off D1 sections.
