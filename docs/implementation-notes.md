...
Ten tests in `resolveContentLinks.test.ts`, including idempotency and the `wp-content` and lookalike-host cases. Not verified against live od-dev — the host was unreachable (TCP timeout on both HTTPS and SSH) while this landed; the fixture is the `wp:query` markup measured the same day.
