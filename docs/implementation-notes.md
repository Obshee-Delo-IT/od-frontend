...
- **`/contacts/sverdlovskaya/` has no card** (it never had an «Об отделении» accordion for the earlier pass to convert), so its spoiler is a summary and a link. That is the truth about that page, and it is what the fallback renders.
