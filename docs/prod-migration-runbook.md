...
**4.7 Remaining CI work.** `.github/workflows/ci.yml` runs `next typegen` → `lint` → `type-check` → `test` → `build` and **stops there** — the docker build + push-to-GHCR step is still to add. ⚠ **It needs five build-args, not two, and two of them are secrets** (§4.5): `WP_BASE`, `WP_MEDIA_CDN` and `SITE_URL` because they are evaluated at build time, plus `WP_USER` and `WP_PASSWORD` or the pushed image ships an empty header and footer on every prerendered page. CI's *verification* build stays secretless on purpose — that is a virtue, and it is why this was never caught there — but the image that deploys cannot be.

~~⚠️ **The Dockerfile can't accept those build-args yet.**~~ **Fixed 2026-08-21** — the `builder` stage now declares all five, and `docker-compose-prod.yaml` passes them from whatever file `--env-file` names, so one flag points both the build and the runtime at the same tier (`ENV_FILE=.env.stage docker compose --env-file .env.stage -f docker-compose-prod.yaml up -d --build`). The warning was right about the consequence: without `WP_BASE` at build time `next.config.ts` falls back to `https://wp.invalid` and every `next/image` request 400s.
