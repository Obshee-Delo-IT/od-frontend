...
3. **Application password + `wp-openapi`,** then `pnpm generate:types` against stage and diff it. od-dev is WP **6.8.8** and stage is now **7.1** — the REST payload shape across that gap is unverified, and the generated types are how we'd find out.
4. **Category ids (B5).** §1.3 against stage, record the real numbers and counts, then `src/shared/config/filmCategories.ts`, `newsCategories.ts` and `scripts/lib/wp.mjs` (which keeps its own copy).
5. **Media origin (B6).** Stage inherited prod's `.htaccess` 301s for uploads 2009–2023 → `obshee-delo.website.yandexcloud.net`; confirm `WP_MEDIA_CDN` matches that bucket and that 2024+ uploads still resolve locally.
6. **Menus.** `wp/v2/menus` and `menu-items` are core-`show_in_rest` since 5.9, but the frontend has only ever read them from od-dev — probe them on stage now that the nav plugins are gone.
7. **`od-revalidate.php` (B4)** — §2.5, installed inert until a frontend tier points at stage.
