...
> | 9 | Re-register what the theme owned (**§0.6** item 6) | `wp term update nav_menu 39 --slug=main-navigation`, plus [`od-sidebars.php`](../wp/mu-plugins/od-sidebars.php) and `wp widget move text-4 text-2 text-3 text-5 --sidebar-id=sidebar_bottom --position=N`. **Not optional and easy to miss:** without them the header and the footer both render *empty*, at 200, because the frontend looks up the nav by a slug only od-dev carries and the footer by a widget area only `welfare` registered. |
