...
> | 8 | Plugin prune ([`wp-backend.md` §4.3](./wp-backend.md)), **leyka included (§2.10)** | After the conversion, so a plugin whose shortcodes are still in a body is measured before it is dropped, not after. |
...
**2.10 Turn `leyka` off — donations do not come back through WordPress.** Deactivate **and delete** it on the new install. **Already done on od-stage**, where it has been `inactive` since before 2026-08-24; the new install needs the same plus the delete. The rule is the one [`next-steps.md`](./next-steps.md) states for the newsletter too: money and personal data must not be routed through an install nobody administers.

**The numbers behind it, read off prod 2026-08-24.** `leyka_donation` holds **3070** records: **467** `funded`, the last of them **2022-01-05**, and **2603** `submitted` — a count that keeps growing, 200 rows in 2026 alone and ~345 a year since 2021. So people still fill in a donation form that has taken no money in four and a half years, and every attempt leaves a name and an address in `wp_postmeta` (**42 641** rows across those posts). Both «Оказать помощь» CTAs already point at the external donation host and nothing in the frontend reads `leyka_*`. Two smaller reasons worth the line: an active payment plugin keeps `/leyka-process-donation` open on a headless install, and leyka 3.30.3 is one of the two plugins whose PHP 8.2 deprecations make WP-CLI's `--format=csv` unparseable ([`wp-backend.md` §2](./wp-backend.md)) — od-stage's plain `wp` has been clean since it went inactive there.

**Keep the records; only the plugin goes.** Neither deactivation nor deletion touches a row: the 3070 posts and their meta stay in `wp_posts` / `wp_postmeta`, unregistered and out of REST, which is what an archive of donations should be once the plugin is gone. Note what they are *not* in — `wp_leyka_donations` and `wp_leyka_donations_meta` are **empty, 0 rows each on prod and on the clone** (measured 2026-08-24), so the note that used to say those tables "hold donation records" was wrong; they drop with the rest of the orphan tables in §0.6 item 10 and the history is the CPT.

**`/sp/` is the only page that ever carried a form, and it is already dead.** Its body holds `[leyka_campaign_form id=»54235″]` as **literal text** — the conversion turned the shortcode's quotes typographic, so leyka has not parsed it since the migration and this step changes nothing visible. What to do with the page is the open editorial decision in [`next-steps.md`](./next-steps.md).

**2.10 Gate** — plugin gone, records intact, endpoint closed:

```bash
wp plugin list --fields=name,status | grep -i leyka                                   # no output
wp db query 'SELECT COUNT(*) FROM wp_posts WHERE post_type="leyka_donation"'          # 3070
curl -s -o /dev/null -w "%{http_code}\n" https://<the new install>/leyka-process-donation  # 404
```
...
**Then `pnpm film:posters`**, which fills `poster_image_url` — the film page's poster card — from the best source available: the Яндекс.Диск плакат the editor supplied (a file linked in `poster_download_url`, or a named file inside a shared folder listed in the script's `FOLDER_POSTERS` table — **keyed by prod's post ids**, so the same run reproduces those nine posters on prod), else a плакат-named image in the post body, else the featured image. Run it **after** 3.5, since the covers it falls back to are what 3.5 uploads. Dry-run by default; a плакат is never overwritten, a cover written as the fallback is recomputed each run, so a плакат arriving later still wins. ⚠ **It needs an account that can edit.** od-stage's `od-frontend` password is the two-capability role of §2.9 (`read` + `edit_theme_options`), so every write answers `403 rest_cannot_edit` and the run reports «0 set, N failure(s)» — read the summary, not the exit code, if you pipe it through `grep`. Any write pass on that tier (this, `film:import`, `film:covers`) needs an editor or administrator application password.
...
**4.9 Yandex Metrica — the tag moves with the domain, it is not copied.** Counter **34478865**, production's own, and the apex does not change under §0.4 — so the counter keeps its history and there is never a second one. The account is the org's and access is in hand: the «Страницы входа» export that gate 12 replays comes out of it.

**Where the tag is today.** One place: `wp-content/themes/welfare/header.php`, **lines 76–92** (prod, read 2026-08-24), printed right after `wp_head()` — the standard snippet plus the `<noscript>` pixel, initialised with `clickmap`, `trackLinks`, `accurateTrackBounce` and `webvisor` all `true`. Line 24 of the same file carries `<meta name="yandex-verification" content="5970d7ec7d8e8b0b">`, and that one needs no work: the app already emits it from `metadata.verification.yandex` in `app/layout.tsx`.

**What to build.** The same snippet and the same four options in `app/layout.tsx` through `next/script`, `strategy="afterInteractive"`. **Unconditional — not gated on `CookieNotice`.** Production's is unconditional, the published policy offers browser settings as the opt-out (§15.3, [notes §5](./implementation-notes.md)), and F6's notice is a statement about cookies rather than a consent gate — which is exactly parity. Gating Metrica on the click is a *new* decision, not this step.

**Cut it out of the frozen copy at cutover** (§5.5). One counter must have one source, and after cutover that source is the app on the apex. `/legacy/*` already helps: `transformLegacyHtml` strips `mc.yandex.ru` scripts out of every page it proxies, so an embedded page never double-counts and its hits land on the parent route. That covers what comes through us; a direct request to `frozen.obshee-delo.ru` would still write into the counter. §5.6 closes that host to everything but the frontend — deleting the block is what makes it true no matter how that allowlist drifts later.

⚠ **Expect the numbers to step up, and do not read it as double counting.** On prod the snippet is `type="rocketlazyloadscript"`: WP Rocket's delay-JS rewrites it, so the counter fires only after the visitor's first interaction and a visit with no interaction sends **nothing at all** — the same trap that kept the cookie notice invisible ([notes §5](./implementation-notes.md)). `next/script` has no such delay. Every traffic figure in these docs, gate 12's entry visits included, comes from the delayed counter.

**4.9 Gate** — one tag, one source:

```bash
curl -s https://obshee-delo.ru/ | grep -c 'mc.yandex.ru/metrika/tag.js'   # 1
curl -s https://obshee-delo.ru/get-involved/ | grep -c '34478865'          # 1 — the app's; the iframe's own copy is stripped
curl -s https://frozen.obshee-delo.ru/ | grep -c '34478865'               # 0
```

Then Метрика → Отчёты → «В реальном времени»: hits arriving, host = the apex.
...
**On the old install** — the same four, pointed at `frozen.obshee-delo.ru`, **plus one deletion: cut the Metrica counter out of `wp-content/themes/welfare/header.php`** (lines 76–92, §4.9). The tag *moves* to the app; a copy left running on the subdomain writes a second stream into the same counter. Until step 2 is done there, the frozen copy 301s every request to the apex, which is now the frontend, which fetches the frozen copy: a loop. `src/shared/legacy/legacyOrigin.ts` warns when `WP_LEGACY_BASE` is the site's own origin, but nothing warns about this direction. **And leave that install on PHP 7** — see §2.7.
...
- **A4 Yandex Metrica** — the counter is **34478865** (read off prod's live tag), the account is in hand, and the procedure is now **§4.9**: the same snippet in `app/layout.tsx`, unconditional as prod's is, and cut out of the frozen copy's theme at cutover (§5.5). What is left is the build itself. ~~The consent banner~~ ✅ **shipped 2026-08-24** — `CookieNotice`, prod's copy and cookie in a corner card ([notes §5](./implementation-notes.md#f6b-the-cookie-notice-in-the-app--2026-08-24)); it states, it does not gate. ~~**F6** 152-FZ privacy page~~ ✅ **done on prod 2026-08-13**, not ported into the repo: `/conf_politics/` is Tier 4, so the A6 fallback serves prod's own page and prod is where the text was corrected (notes §5). The СМИ registration line and 12+ badge come from od-dev widget `block-27` and are already rendered by C9's footer — but **two hrefs in that widget break on this origin**, see F6 in the plan. **A2 is decided** — Beget VPS + Coolify — but the deploy half of **A3** (docker build + push to GHCR, incl. the Dockerfile build-args in §4.7) is still open.
