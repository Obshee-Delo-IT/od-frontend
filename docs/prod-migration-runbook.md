...
The stored value is the **short id from `play_link`** (`https://kinescope.io/<short>`), because that is what `FilmPlayer` embeds. The API's `id` field is a different UUID and must **not** be used. Matching is deliberately conservative and refuses to guess: trailers/teasers and numbered course lessons are excluded, and where a download label carries a duration it is checked against the Kinescope duration — a mismatch is reported rather than written. Re-run `film:import` afterwards to push the new ids. On od-dev this took `kinescope_id` from **1 → 62 of 99**.
...
- **`kinescope_id`: 62 of 99 after §3.6.** The 37 without one fall back to poster → `watch_url` → bare poster. Almost all are «Известные люди» short interviews that were never uploaded to Kinescope. Specific cases:
  - **67400 «Курение. Взгляд изнутри»** — the source video **is** in Kinescope (`2kgaX4mQMQcxVvn177pfDF`, exact title) but its `status` is `error` with duration 0, a failed transcode. **Re-upload it in the Kinescope dashboard**, then re-run §3.6.
  - **19871 «Секреты манипуляции — Алкоголь»** — two candidates, no `share_youtube` to break the tie. Add the YouTube link and re-run.
  - **28749 «The Mystery of the Deadly Smoke»** — the English cut. Three Russian versions of «Тайна едкого дыма» exist; no English one.
  - **37626 / 14590 / 32168** duplicate other WP posts (26122 / 38406 / 38424). Reusing one Kinescope id across two posts is fine, but merging the posts is the better fix.
  - The library also holds **duplicate uploads**: 70570 «Деньги с дымком» matches both `5jKB95R3giLJfNn1f6ZKGf` (plain title, currently stored and verified playing) and `wjvSkxqQjW9dRaHpdJ93aj` (the YouTube-imported marketing title). Harmless, but it's why the bridge reported one disagreement out of 17.
- **`watch_url`: 0 of 99.**
