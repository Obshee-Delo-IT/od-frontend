...
- **Given** the image the `image` job of `.github/workflows/ci.yml` builds and pushes — `ghcr.io/obshee-delo-it/od-frontend:stage` — which receives `WP_USER` and `WP_PASSWORD` as **BuildKit secrets** mounted onto the builder's `RUN pnpm build` (the job's `secrets:` → the Dockerfile's `--mount=type=secret`); they were build-args until 2026-08-23, see SEC-01b. Two needles: the plaintext application password, and `base64("$WP_USER:$WP_PASSWORD")`, which is the form `src/shared/api/httpClient.ts:20` actually stores.
...
- **Why:** the Dockerfile's builder stage says the credentials never leave it — which is true of the *layers* — but `pnpm build` writes a copy of `.env` into the standalone output — measured in this repo today, `grep -rlF "$WP_PASSWORD" .next` returns exactly one file, `.next/standalone/.env` — and `Dockerfile:71-73` does `COPY --from=builder /app/.next/standalone ./`. The only thing keeping the WP application password out of a published GHCR layer is the `.env` / `.env.*` pair in `.dockerignore`, whose own comment says why. Deleting either line bakes the credential into the image with every gate still green. · **Covered by:** **gap**

#### SEC-01b · P0 · infra — Prove no credential reaches the published build record

- **Given** the repository is **public**, so `GET /repos/Obshee-Delo-IT/od-frontend/actions/artifacts` is listable with no account and the `/zip` download is open to any authenticated GitHub user. `docker/build-push-action@v6` uploads a BuildKit build record as a `*.dockerbuild` artifact by default, and that record carries every build arg verbatim in `frontendAttrs` — plus, separately, in an SLSA predicate embedded in the same bundle, which is why `provenance: false` did not cover it.
- **When** you download the newest `.dockerbuild` artifact and grep the decompressed bundle for each build-arg key.
- **Then** `build-arg:WP_PASSWORD` and `build-arg:WP_USER` return **0** matches. `build-arg:WP_BASE`, `build-arg:SITE_URL` and `build-arg:WP_MEDIA_CDN` may match — they are `vars`, not secrets. If the job keeps `DOCKER_BUILD_RECORD_UPLOAD: false`, there is no artifact to grep at all, which also passes.
- **Verify:** `ID=$(gh api repos/Obshee-Delo-IT/od-frontend/actions/artifacts --jq '.artifacts[0].id')` · `gh api "repos/Obshee-Delo-IT/od-frontend/actions/artifacts/$ID/zip" > r.gz` · `zcat r.gz | grep -ac 'build-arg:WP_PASSWORD'` → 0. Count only; never print the value.
- **Why:** this is the leak, not a hypothetical. From 2026-08-21 to 2026-08-23 the two credentials were `build-args:`, and nine unexpired artifacts each held `"build-arg:WP_PASSWORD": "<the od-stage application password>"` in cleartext — one per push to `main`, retained 90 days. The account is `od-frontend` (id 5367, administrator on `https://od.webtm.ru`), so the artifact granted authenticated REST write access and, through plugin upload, a path to RCE on the prod clone. Found by a full-repo security review 2026-08-22; the password was rotated, the artifacts deleted, and the credentials moved to `RUN --mount=type=secret`. · **Covered by:** **gap**
...
- **Given** `ghcr.io/obshee-delo-it/od-frontend` is a PUBLIC package (which is why the VPS needs no registry credentials), and `WP_USER`/`WP_PASSWORD` reach the build as BuildKit secrets mounted onto one `RUN`, never as `ARG`+`ENV`.
