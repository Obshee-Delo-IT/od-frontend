...
A probe of od-dev found **174 published `wp/v2/pages`** (re-confirmed 2026-08-13; **169** as of 2026-08-20, after the WooCommerce group and `/materials/order-materials/` were deleted) — regional pages (~40), programmes (`/projects`, `/healthy-russia`, …), materials (`/printed-products`, `/social-reklama`, …), legal (`/conf_politics`, `/rekvizit`, …), team, plus a long tail. The Figma redesign covers a fraction (D1–D9). Redesigning all of them before launch isn't realistic, and "build everything, publish once at the end" is too slow — so we publish now and let un-redesigned pages fall back to their embedded old content, replacing them one at a time.

**How much traffic this actually carries — measured, not assumed.** *Superseded by [`page-inventory.md` §6](./page-inventory.md#6-traffic-re-measured): re-measured 2026-08-20, the iframe is 7 paths carrying **0.8 % of entries and 2.6 % of views**, because D3 and D6 moved ~150 pages onto native rendering at the same URLs. The two paragraphs below are the 2026-08-13 measurement, kept because the tiering was priced off them.* 91 days of Yandex Metrica put every page that would sit behind the fallback at **13.5 % of entry visits**; the redesigned routes cover 85.1 % and the legacy redirects another 1.5 %. That reframes the SEO trade in §4 from "risky" to "priced": it is 13.5 % of entries on pages we are actively replacing. It also says *which* of them shouldn't wait — `/materials/plakati/` alone is the #6 entry page on the whole site and deserves a native route before prod, not an iframe. The tiering is in [`implementation-plan.md` → Launch priority](./implementation-plan.md#launch-priority).
