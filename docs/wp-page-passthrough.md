...
2. `isLegacyEmbedPage(path)` — the opt-out list in `shared/config/legacyEmbedPages.ts` (**7** paths as of 2026-08-20 — it was 20 before workstream D took pages off it one at a time). A listed path goes straight to the iframe.
...
**Opt-out, not opt-in.** 153 of the 169 published pages render through `WpPage` — 118 of them with nothing configured at all, 35 with their content rewritten by `od-pages.php` (the other 16 are the 7 on the opt-out list and 9 shadowed by a native route; see [`page-inventory.md`](./page-inventory.md)), a newly published WP page is live at its URL without a deploy, and no URL changes — entries and rankings are untouched. The cost is one WP request on paths that turn out not to be pages, cached for an hour under the `wp:pages` tag and bounded by the route's ISR window.
...
- **`/materials/order-materials/`** (id 22125, 19 views) — a «Корзина заказов» button to `/cart/`, a `[recent_products]` shortcode and its own «В данное время заказ материалов не осуществляется». **Deleted on prod, then trashed on od-dev the same day** (`wp post delete 22125`), so the button went with the page and its `LEGACY_EMBED_PAGES` entry is gone too. The order mattered: with the od-dev copy still published, dropping the entry first would have rendered that page natively, dead button and all.
...
**WooCommerce the plugin is already gone** where it matters — verified 2026-08-17: no `woocommerce` entry and no plugin directory on live prod (BeGet) or od-dev. The last copy on the account was **od-stage**, a stale clone of an older prod — and it went with that tier's re-sync: od-stage was rebuilt from live prod on 2026-08-20 and its WooCommerce tables and plugin are gone with it. What remains on prod is the *theme's* WooCommerce code (`welfare/functions.php` guards it behind `CMSMS_WOOCOMMERCE`) — see §2 there for why it makes WP-CLI need `--skip-themes`.
