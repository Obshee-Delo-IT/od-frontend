...
const previewSource = (target: EventTarget | null): PreviewSource | null => {
  ...
  // A pointer click lands on the `<img>`; a click generated by Enter on the
  // focused anchor lands on the **anchor**, where `closest('img')` finds
  // nothing — so the keyboard path fell through this guard, `open` bailed
  // without `preventDefault`, and the browser navigated to the raw JPEG
  // instead of opening the lightbox (A11Y-12).
  const anchor = target.closest('a');
  const img = target.closest('img') ?? anchor?.querySelector('img') ?? null;
  if (!(img instanceof HTMLImageElement) || !img.currentSrc) {
    return null;
  }
  const href = (anchor ?? img.closest('a'))?.getAttribute('href') ?? null;
  ...
  return (
    <>
      {/* The keyboard path is the anchor WordPress already wraps the image in:
          it is focusable, and activating it fires a click this handler sees —
          on the anchor rather than on the image, which is what `previewSource`
          accounts for. An image with no link is decoration and stays inert,
