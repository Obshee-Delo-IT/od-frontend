...
Outside `src/`, **`wp/` holds PHP that runs on the WordPress side**, and every directory under it is the canonical copy — edit here, re-upload with `scp`, and keep the doc that describes each pointing at the file rather than embedding a second copy. `wp/mu-plugins/od-revalidate.php` is the `save_post`-side half of B4, installed on od-dev at `wp-content/mu-plugins/` (`docs/wp-backend.md` §6.5). `wp/plugins/cmsms-gutenberg-upgrade/` is the shortcode→Gutenberg migrator, ours to maintain until cmsms is removed after cutover; it carries a `wp cmsms` WP-CLI command so a migration can be run and diffed from a script ([`docs/wp-page-passthrough.md`](docs/wp-page-passthrough.md) §6). `wp/scripts/od-pages.php` is workstream D's content fixes — run with `wp eval-file`, dry-run unless given a positional `apply`, and its pure transforms are tested by `php wp/tests/od-pages.test.php` against real bodies in `wp/tests/__fixtures__/`. **Mind the PHP floor:** an mu-plugin loads on every *site* request, where prod is still `mod_php7`, so `od-revalidate.php` is PHP 7.0 syntax only; `od-pages.php` runs under CLI PHP 8.2 and can use anything. See [`docs/wp-page-redesign.md`](docs/wp-page-redesign.md).
